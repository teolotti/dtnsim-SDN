...
static sm_SemId _sdrlock(int delete_)
{
...
static SdrControlHeader *_sch(SdrControlHeader **schp)
{
	...
	if (*schp) /*	Shutdown.		*/
	{
		...
		if (lock != SM_SEM_NONE)
		{
			...
			sm_SemTake(lock);
			for (elt = sm_list_first(sdrwm, sch->sdrs); elt; elt = sm_list_next(sdrwm, elt))
			{
...
static PsmPartition _sdrwm(sm_WmParms *parms)
{
	...
	if (parms)
	{
		...
		if (memmgr_open(parms->wmKey, parms->wmSize, &parms->wmAddress, &sdrwmId, parms->wmName, &sdrwm, &memmgrIdx, sdrmtake, sdrmrlse, sdrmatop, sdrmptoa) < 0)
		{
...
int takeSdr(SdrState *sdr)
{
	...
	if (sdr->sdrOwnerTask == sm_TaskIdSelf() && pthread_equal(sdr->sdrOwnerThread, pthread_self()))
	{
		sdr->xnDepth++;
...
{
	_postErrmsg(fileName, lineNbr, "Assertion failed, SDR transaction canceled.", arg);
	writeErrmsgMemos();
	crashXn(sdrv);
...
static int readFromLog(int logfile, char *logsm, unsigned long offset, char *into, size_t length, SdrState *sdr)
{
	if (logsm == NULL) /*	Log is in file.		*/
	{
		if (lseek(logfile, offset, SEEK_SET) < 0 || read(logfile, into, length) < length)
		{
	...
	else /*	Log is in memory.	*/
	{
		...
		{
			putErrmsg("Log entry extends past end of log.", itoa(length));
			return -1;
		}
...
static int reverseTransaction(SdrState *sdr, int logfile, char *logsm, int dsfile, char *dssm)
{
	PsmPartition sdrwm = _sdrwm(NULL);
	PsmAddress elt;
	size_t length;
	...
	for (elt = sm_list_last(sdrwm, sdr->logEntries); elt; elt = sm_list_prev(sdrwm, elt))
	{
		logEntryOffset = (unsigned long) sm_list_data(sdrwm, elt);
		length = sizeof logEntryControl;
		if (readFromLog(logfile, logsm, logEntryOffset, (char *) logEntryControl, length, sdr) < 0)
		{
		...
		if (dssm)
		{
			if (readFromLog(logfile, logsm, logEntryOffset, dssm + logEntryControl[0], length, sdr) < 0)
			{
			...
			if (dsfile != -1)
			{
				...
				if (lseek(dsfile, logEntryControl[0], SEEK_SET) < 0 || write(dsfile, dssm + logEntryControl[0], length) < length)
				{
		...
		else /*	Must create buffer for recovered data.	*/
		{
			if (dsfile != -1)
			{
				buf = (char *) MTAKE(length);
				if (buf == NULL)
				{
					putErrmsg("Log entry too big.", itoa(length));
					return -1;
				}

				if (readFromLog(logfile, logsm, logEntryOffset, buf, length, sdr) < 0)
				{
					putErrmsg("Can't read log entry.", NULL);
					MRELEASE(buf);
					return -1;
				...
				if (lseek(dsfile, logEntryControl[0], SEEK_SET) < 0 || write(dsfile, buf, length) < length)
				{
					putSysErrmsg("Can't reverse log entry", NULL);
					MRELEASE(buf);
					return -1;
...
static void clearTransaction(Sdr sdrv)
{
	...
	if (sdrv->sdr->configFlags & SDR_REVERSIBLE)
	{
		...
		{
			isprintf(logfilename, sizeof logfilename, "%s%c%s.sdrlog", sdrv->sdr->pathName, ION_PATH_DELIMITER, sdrv->sdr->name);
			sdrv->logfile = iopen(logfilename, O_RDWR | O_CREAT | O_TRUNC, 0777);
			if (sdrv->logfile == -1)
			{
...
static void terminateXn(Sdr sdrv)
{
	...
	if (reverseTransaction(sdr, sdrv->logfile, sdrv->logsm, sdrv->dsfile, sdrv->dssm) < 0)
	{
		handleUnrecoverableError(sdrv);
	...
	{
		writeMemoNote("[!] Can't execute restart command", sdr->restartCmd);
		sdr->halted = 0;
		clearTransaction(sdrv);
		unlockSdr(sdr);
		return;
...
void crashXn(Sdr sdrv)
{
	...
	if (sdr_in_xn(sdrv))
	{
		sdr = sdrv->sdr;
#ifdef SDR_TRACE
		sptrace_log_memo(sdrv->trace, 0, "transaction aborted", sdrv->currentSourceFileName, sdrv->currentSourceFileLine);
#endif
...
static int reloadLogEntries(SdrState *sdr, int logfile)
{
	...
	while (1)
	{
		...
		lengthRead = read(logfile, (char *) logEntryControl, sizeof logEntryControl);
		...
		endOfEntry = sdr->logLength + sizeof logEntryControl + logEntryControl[1];
...
static int createDsFile(SdrState *sdr, char *dsfilename)
{
	...
	MRELEASE(buffer);
	initSdrMap(&map, sdr);
	if (lseek(dsfile, 0, SEEK_SET) < 0 || write(dsfile, (char *) &map, sizeof map) < sizeof map || lseek(dsfile, 0, SEEK_SET) < 0)
	{
		close(dsfile);
		unlink(dsfilename);
...
static void destroySdr(SdrState *sdr)
{
	...
	{
		isprintf(dsfilename, sizeof dsfilename, "%s%c%s.sdr", sdr->pathName, ION_PATH_DELIMITER, sdr->name);
		unlink(dsfilename);
	}
	...
	if (sdr->configFlags & SDR_REVERSIBLE)
	{
		...
		{
			isprintf(logfilename, sizeof logfilename, "%s%c%s.sdrlog", sdr->pathName, ION_PATH_DELIMITER, sdr->name);
			unlink(logfilename);
		}
		else /*	Log is in memory.	*/
		{
			switch (sm_ShmAttach(sdr->logKey, sdr->logSize, &logsm, &logsmId))
			{
			case -1:
				break;
...
int sdr_load_profile(char *name, int configFlags, long heapWords, int heapKey, int logSize, int logKey, char *pathName, char *restartCmd)
{
	sm_SemId lock = _sdrlock(0);
	PsmPartition sdrwm = _sdrwm(NULL);
	...
	{
		putErrmsg("No SDR heap site specified in configFlags.", itoa(configFlags));
		return -1;
	}
#if (HEAP_PTRS)
...
SDR heap data, the heap MUST be resident in memory.", itoa(configFlags));
	...
	for (elt = sm_list_first(sdrwm, sch->sdrs); elt; elt = sm_list_next(sdrwm, elt))
	{
		...
		{
			sm_SemGive(lock);
			if (sdr->configFlags == configFlags && sdr->initHeapWords == heapWords && (sdr->dsKey == heapKey || heapKey == SM_NO_KEY) && sdr->logSize == logSize && (sdr->logKey == logKey || logKey == SM_NO_KEY)
	...
	if (stat(sdr->pathName, &statbuf) < 0 || (statbuf.st_mode & S_IFDIR) == 0)
	{
...
in file and transaction reversibility", sdr->pathName);
	...
	if (sdr->configFlags & SDR_REVERSIBLE)
	{
		...
		{
			logsm = NULL;
			if (sm_ShmAttach(sdr->logKey, sdr->logSize, &logsm, &logsmId) < 0)
			{
		...
		{
			isprintf(logfilename, sizeof logfilename, "%s%c%s.sdrlog", sdr->pathName, ION_PATH_DELIMITER, name);
	...
	if (sdr->configFlags & SDR_IN_FILE)
	{
		isprintf(dsfilename, sizeof dsfilename, "%s%c%s.sdr", sdr->pathName, ION_PATH_DELIMITER, name);
		...
		if (dsfile == -1) /*	No dataspace file.	*/
		{
			...
			if (dsfile == -1) /*	Can't create.	*/
			{
				...
				if (logsm)
					sm_ShmDetach(logsm);
				putErrmsg("Can't have file-based dataspace", NULL);
				destroySdr(sdr);/*	Releases lock.	*/
				return -1;
			}
		}
	...
	if (sdr->configFlags & SDR_IN_DRAM)
	{
		...
		case 0: /*	Reattaching to existing SDR.	*/
			...
			if (reverseTransaction(sdr, logfile, logsm, -1, dssm) < 0)
			{
				sm_ShmDetach(dssm);
				if (dsfile != -1)
					close(dsfile);
...
int sdr_reload_profile(char *name, int configFlags, long heapWords, int heapKey, int logSize, int logKey, char *pathName, char *restartCmd)
{
	sm_SemId lock = _sdrlock(0);
	PsmPartition sdrwm = _sdrwm(NULL);
	SdrControlHeader *sch = _sch(NULL);
	...
	sm_SemGive(lock);
	return sdr_load_profile(name, configFlags, heapWords, heapKey, logSize, logKey, pathName, restartCmd);
}
...
Sdr Sdr_start_using(char *name)
{
	...
	{
		sdrv->dssm = NULL;
		if (sm_ShmAttach(sdr->dsKey, sdr->dsSize, &(sdrv->dssm), &(sdrv->dssmId)) < 0)
		{
			sm_SemGive(lock);
			putErrmsg("Can't attach to dataspace in memory.", itoa(sdr->dsKey));
			return NULL;
		}
	}
	...
	if (sdr->configFlags & SDR_REVERSIBLE)
	{
		...
		{
			isprintf(logfilename, sizeof logfilename, "%s%c%s.sdrlog", sdr->pathName, ION_PATH_DELIMITER, name);
			sdrv->logfile = iopen(logfilename, O_RDWR | O_CREAT | O_APPEND, 0777);
			if (sdrv->logfile == -1)
			{
				sm_SemGive(lock);
				putSysErrmsg("Can't open log file", logfilename);
		...
		else /*	Log is in memory.	*/
		{
			sdrv->logfile = -1;
			sdrv->logsm = NULL;
			if (sm_ShmAttach(sdr->logKey, sdr->logSize, &(sdrv->logsm), &(sdrv->logsmId)) < 0)
			{
				sm_SemGive(lock);
				putErrmsg("Can't attach to log in memory.", itoa(sdr->logKey));
				return NULL;
			}
		}
	}
...
//	puts("#####sdr_in_xn()");
//	bool cond1 = (sdrv->sdr != NULL);
//	bool cond2 = (sdrv->sdr->sdrOwnerTask == sm_TaskIdSelf());
//	bool cond3 = (pthread_equal(sdrv->sdr->sdrOwnerThread, pthread_self()));
//	printf("cond1 = %d\n",cond1);
//	printf("cond2 = %d\n",cond2);
//	printf("cond3 = %d\n",cond3);
//	printf("(sdrv->sdr->sdrOwnerTask = %d\n", (sdrv->sdr->sdrOwnerTask));
//	printf("sm_TaskIdSelf() = %d\n", sm_TaskIdSelf());

	return (sdrv->sdr != NULL && sdrv->sdr->sdrOwnerTask == sm_TaskIdSelf() && pthread_equal(sdrv->sdr->sdrOwnerThread, pthread_self()));
}
...
static int writeToLog(const char *file, int line, Sdr sdrv, char *from, long length)
{
	SdrState *sdr = sdrv->sdr;
...
void _sdrput(const char *file, int line, Sdr sdrv, Address into, char *from, long length, PutSrc src)
{
	SdrState *sdr;
	Address to;
	...
	if (sdr->configFlags & SDR_REVERSIBLE)
	{
		...
		if (writeToLog(file, line, sdrv, (char *) logEntryControl, sizeof logEntryControl) < 0)
		{
		...
		if (sdr->configFlags & SDR_IN_DRAM)
		{
			if (writeToLog(file, line, sdrv, sdrv->dssm + into, length) < 0)
			{
...
log entry.", itoa(length));
			...
			if (lseek(sdrv->dsfile, into, SEEK_SET) < 0 || read(sdrv->dsfile, buffer, length) < length)
			{
				MRELEASE(buffer);
...
entry.", NULL);
	...
	if (sdr->configFlags & SDR_IN_FILE)
	{
		if (lseek(sdrv->dsfile, into, SEEK_SET) < 0 || write(sdrv->dsfile, from, length) < length)
		{
...
void Sdr_write(const char *file, int line, Sdr sdrv, Address into, char *from, long length)
{
	if (!(sdr_in_xn(sdrv)))
	{
...
void _sdrfetch(Sdr sdrv, char *into, Address from, long length)
{
	...
	else
	{
		if (sdr->configFlags & SDR_IN_FILE)
		{
			if (lseek(sdrv->dsfile, from, SEEK_SET) < 0 || read(sdrv->dsfile, into, length) < length)
			{
				putSysErrmsg("Dataspace read failed", itoa(length));
				crashXn(sdrv); /*	Releases SDR.	*/
				return;
			}
		}
