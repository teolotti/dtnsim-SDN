...
as expected.", NULL);
    ...
    srcNode = (char *) MTAKE(MAX_SCHEME_NAME_LEN + 1 + MAX_EID_LEN);
    destNode = (char *) MTAKE(MAX_SCHEME_NAME_LEN + 1 + MAX_EID_LEN);
    ...
    if((asb.resultData = (unsigned char *) MTAKE(sessionKeyLen + 1 + sessionKeySdnv.length)) == NULL)
    {
...
{
    BspPayloadReplaceKit *bprk = (BspPayloadReplaceKit *) MTAKE(sizeof(BspPayloadReplaceKit));
    CHKNULL(bprk);
                                     ...
                                     unsigned char *sessionKeyValue, unsigned int sessionKeyLen)   
{
    ...
    if(bprk->headerLen > 0) {
        ...
        bprk->newBundle = zco_create(bpSdr, ZcoSdrSource, newSdrAddr,
                                         0, bprk->headerLen, ZcoInbound, provisional);
    }
    else
    {
        bprk->newBundle = zco_create(bpSdr, ZcoSdrSource, 0, 0, 0, ZcoInbound, provisional);
    }
...
{ 
    ...
    if(bprk->headerLen > 0)
    {
        bprk->headerBuff = (char *) MTAKE(bprk->headerLen);
    ...
    if(bprk->trailerLen > 0)
    {
        bprk->trailerBuff = (char *) MTAKE(bprk->trailerLen);
                  ...
                  
{
   ...
   dataBuffer = (unsigned char *) MTAKE(PCB_ENCRYPTION_CHUNK_SIZE);
   CHKERR(dataBuffer);
...
{
    unsigned char *encryptedKey = (unsigned char *) MTAKE(sessionKeyLen);
...
unsigned char *bsp_pcbGenSessionKey(unsigned int *sessionKeyLen) 
{
    ...
    unsigned char *sessionKey = (unsigned char *) MTAKE(PCB_SESSION_KEY_LENGTH + 1);
    CHKNULL(sessionKey);
