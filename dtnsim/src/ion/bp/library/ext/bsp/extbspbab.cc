			...
			Bundle *bundle, void *ctxt)
{
    ...
    */
   if((asb.resultData = (unsigned char *) MTAKE(digestLen + digestOffset)) == NULL)
   {
...
serializing block. Result is %d", result);
   ...
   if((temp = (char *) MTAKE(blk->length)) == NULL)
   {
                                   ...
                                   unsigned int *hashLen)
{
   ...
   dataBuffer = (char *) MTAKE(BSP_BAB_BLOCKING_SIZE);
   CHKNULL(dataBuffer);
...
//   }
   ...
   if((authContext = (char *) MTAKE(authCtxLen)) == NULL)
   {
   ...
   if((hashData = (unsigned char *) MTAKE(BAB_HMAC_SHA1_RESULT_LEN)) == NULL)
   {
