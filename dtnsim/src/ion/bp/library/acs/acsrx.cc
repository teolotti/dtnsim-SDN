...
{	
	...
	{
		fillElt = (AcsFill *) lyst_data(fillLElt);
		MRELEASE(fillElt);
...
{
	char	*reprAcs = (char *) MTAKE(MAX_REPRACS_LEN + 4);
	char	*cursor = reprAcs;		
	char	*toReturn;
	int		rc = 0;
	...
	toReturn = (char *) MTAKE(MAX_REPRACS_LEN - reprAcsLeft + 1);
			...
			int unparsedBytes, int bundleIsFragment)
{
	...
	acs->reasonCode = (BpCtReason) (*cursor & 0x7f);
	cursor++;
	unparsedBytes--;
	...
	{
		fillElt = (AcsFill *) MTAKE(sizeof(AcsFill));
		if (fillElt == 0)
		{
