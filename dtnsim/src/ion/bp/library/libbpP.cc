...
static int offerNoteAcs(Bundle *b, AcqWorkArea *w, char *d, int s, BpCtReason r)
{
	return 0;
}

static int parseACS(int adminRecordType, void **otherPtr, unsigned char *cursor, int unparsedBytes, int bundleIsFragment)
{
	return -2;
}

static int applyACS(int adminRecType, void *other, BpDelivery *dlv, CtSignalCB handleCtSignal)
{
	return -2;
}
...
static int parseImcPetition(int adminRecordType, void **otherPtr, unsigned char *cursor, int unparsedBytes)
{
	return -2;
}
...
static BpDB *_bpConstants()
{
	...
	if (db == NULL)
	{
		...
		if (dbObject)
		{
			...
			{
				sdr_read(sdr, (char *) &buf, dbObject, sizeof(BpDB));
			}
			else
			{
...
void bpSourceTally(unsigned int priority, unsigned int size)
{
	...
	tally->currentBytes += size;
	offset = (char *) tally - ((char *) &stats);
	sdr_write(sdr, vdb->sourceStats + offset, (char * ) tally, sizeof(Tally));
}
...
void bpDiscardTally(unsigned int priority, unsigned int size)
{
	...
	tally->currentBytes += size;
	offset = (char *) tally - ((char *) &stats);
	sdr_write(sdr, vdb->discardStats + offset, (char * ) tally, sizeof(Tally));
}
...
static void resetScheme(VScheme *vscheme)
{
	...
	for (elt = sm_list_first(bpwm, vscheme->endpoints); elt; elt = sm_list_next(bpwm, elt))
	{
...
static int raiseScheme(Object schemeElt, BpVdb *bpvdb)
{
	...
	for (elt = sdr_list_first(bpSdr, scheme.endpoints); elt; elt = sdr_list_next(bpSdr, elt))
	{
		if (raiseEndpoint(vscheme, elt) < 0)
		{
			oK(sm_list_destroy(bpwm, vscheme->endpoints, NULL, NULL));
			oK(sm_list_delete(bpwm, vschemeElt, NULL, NULL));
			psm_free(bpwm, addr);
			return -1;
		}
...
static void startScheme(VScheme *vscheme)
{
	...
	if (isUnicast(vscheme->name))
	{
		...
		{
			isprintf(vscheme->adminEid, sizeof vscheme->adminEid, "%.8s:" UVAST_FIELDSPEC ".0", vscheme->name, getOwnNodeNbr());
		}
...
#endif
			isprintf(vscheme->adminEid, sizeof vscheme->adminEid, "%.15s://%.60s.dtn", vscheme->name, hostNameBuf);
		}

		if (parseEidString(vscheme->adminEid, &metaEid, &vscheme2, &vschemeElt) == 0)
		{
			restoreEidString(&metaEid);
			writeMemoNote("[?] Malformed admin EID string", vscheme->adminEid);
		...
		else
		{
			...
			findEndpoint(vscheme->name, metaEid.nss, vscheme, &vpoint, &vpointElt);
			restoreEidString(&metaEid);
			if (vpointElt == 0)
			{
				if (addEndpoint(vscheme->adminEid, EnqueueBundle, NULL) < 1)
				{
					restoreEidString(&metaEid);
...
endpoint", vscheme->adminEid);
	...
	sdr_read(bpSdr, (char *) &scheme, sdr_list_data(bpSdr, vscheme->schemeElt), sizeof(Scheme));
	if (scheme.fwdCmd != 0)
	{
...
static void startInduct(VInduct *vduct)
{
	...
	sdr_read(bpSdr, (char *) &induct, sdr_list_data(bpSdr, vduct->inductElt), sizeof(Induct));
	if (induct.cliCmd != 0)
	{
		sdr_string_read(bpSdr, cmd, induct.cliCmd);
		isprintf(cmdString, sizeof cmdString, "%s %s", cmd, induct.name);
...
static void startOutduct(VOutduct *vduct)
{
	...
	sdr_read(bpSdr, (char *) &outduct, sdr_list_data(bpSdr, vduct->outductElt), sizeof(Outduct));
	if (outduct.cloCmd != 0)
	{
		sdr_string_read(bpSdr, cmd, outduct.cloCmd);
		isprintf(cmdString, sizeof cmdString, "%s %s", cmd, outduct.name);
...
static int raiseProtocol(Address addr, BpVdb *bpvdb)
{
	...
	for (elt = sdr_list_first(bpSdr, protocol->inducts); elt; elt = sdr_list_next(bpSdr, elt))
	{
	...
	for (elt = sdr_list_first(bpSdr, protocol->outducts); elt; elt = sdr_list_next(bpSdr, elt))
	{
...
int orderBpEvents(PsmPartition partition, PsmAddress nodeData, void *dataBuffer)
{
	Sdr sdr = getIonsdr();
	BpEvent *argEvent;
	Object elt;
	...
	{
		putErrmsg("Error calling smrbt BP timeline compare function.", NULL);
		return 0;
	}

	argEvent = (BpEvent *) dataBuffer;
	elt = (Object) nodeData;
	sdr_read(sdr, (char *) &event, sdr_list_data(sdr, elt), sizeof(BpEvent));
...
static BpVdb *_bpvdb(char **name)
{
	...
	if (name)
	{
		...
		if ((vdb->schemes = sm_list_create(wm)) == 0 || (vdb->inducts = sm_list_create(wm)) == 0 || (vdb->outducts = sm_list_create(wm)) == 0 || (vdb->neighbors = sm_list_create(wm)) == 0 || (vdb->timeline = sm_rbt_create(wm)) == 0 || psm_catlg(wm, *name, vdbAddress) < 0)
		{
			sdr_exit_xn(sdr);
		...
		for (sdrElt = sdr_list_first(sdr, db->schemes); sdrElt; sdrElt = sdr_list_next(sdr, sdrElt))
		{
		...
		for (sdrElt = sdr_list_first(sdr, db->protocols); sdrElt; sdrElt = sdr_list_next(sdr, sdrElt))
		{
		...
		for (sdrElt = sdr_list_first(sdr, (_bpConstants())->timeline); sdrElt; sdrElt = sdr_list_next(sdr, sdrElt))
		{
			addr = sdr_list_data(sdr, sdrElt);
...
int bpInit()
{
	...
	case 0: /*	Not found; must create new DB.	*/
		...
		bpdbBuf.bundles = sdr_hash_create(bpSdr, BUNDLES_HASH_KEY_LEN, BUNDLES_HASH_ENTRIES, BUNDLES_HASH_SEARCH_LEN);
		...
		if (bpdbBuf.sourceStats && bpdbBuf.recvStats && bpdbBuf.discardStats && bpdbBuf.xmitStats && bpdbBuf.delStats && bpdbBuf.ctStats && bpdbBuf.dbStats)
		{
			memset((char *) &cosStatsInit, 0, sizeof(BpCosStats));
			sdr_write(bpSdr, bpdbBuf.sourceStats, (char * ) &cosStatsInit, sizeof(BpCosStats));
			sdr_write(bpSdr, bpdbBuf.recvStats, (char * ) &cosStatsInit, sizeof(BpCosStats));
			sdr_write(bpSdr, bpdbBuf.discardStats, (char * ) &cosStatsInit, sizeof(BpCosStats));
			sdr_write(bpSdr, bpdbBuf.xmitStats, (char * ) &cosStatsInit, sizeof(BpCosStats));
			memset((char *) &delStatsInit, 0, sizeof(BpDelStats));
			sdr_write(bpSdr, bpdbBuf.delStats, (char * ) &delStatsInit, sizeof(BpDelStats));
			memset((char *) &ctStatsInit, 0, sizeof(BpCtStats));
			sdr_write(bpSdr, bpdbBuf.ctStats, (char * ) &ctStatsInit, sizeof(BpCtStats));
			memset((char *) &dbStatsInit, 0, sizeof(BpDbStats));
			sdr_write(bpSdr, bpdbBuf.dbStats, (char * ) &dbStatsInit, sizeof(BpDbStats));
		}
...
int bpStart()
{
	...
	for (elt = sm_list_first(bpwm, bpvdb->schemes); elt; elt = sm_list_next(bpwm, elt))
	{
	...
	for (elt = sm_list_first(bpwm, bpvdb->outducts); elt; elt = sm_list_next(bpwm, elt))
	{
...
void bpStop() /*	Reverses bpStart.		*/
{
	...
	for (zcoElt = sdr_list_first(bpSdr, (_bpConstants())->inboundBundles); zcoElt; zcoElt = nextElt)
	{
...
{
	zco_increase_heap_occupancy(getIonsdr(), bundle->dbOverhead, bundle->acct);
}

void noteBundleRemoved(Bundle *bundle)
{
	zco_reduce_heap_occupancy(getIonsdr(), bundle->dbOverhead, bundle->acct);
}
...
void computePriorClaims(ClProtocol *protocol, Outduct *duct, Bundle *bundle, Scalar *priorClaims, Scalar *totalBacklog)
{
	uvast proximateNodeNbr = atol(duct->name);
	int priorClaimsSize = _sdrStatus_.size[(int)proximateNodeNbr];
    loadScalar(totalBacklog, priorClaimsSize);
	loadScalar(priorClaims, priorClaimsSize);
}
...
char *getBpString(BpString *bpString)
{
	...
	string = (char *) MTAKE(bpString->textLength + 1);
	if (string == NULL)
	{
...
char *retrieveDictionary(Bundle *bundle)
{
	...
	dictionary = (char *) MTAKE(bundle->dictionaryLength);
	if (dictionary == NULL)
	{
		return (char *) bundle; /*	Out of memory.	*/
	}

	sdr_read(getIonsdr(), dictionary, bundle->dictionary, bundle->dictionaryLength);
	return dictionary;
}
...
int parseEidString(char *eidString, MetaEid *metaEid, VScheme **vscheme, PsmAddress *vschemeElt)
{
	...
	*(metaEid->colon) = '\0';
	...
	if (sscanf(metaEid->nss, UVAST_FIELDSPEC ".%u", &(metaEid->nodeNbr), &(metaEid->serviceNbr)) < 2 || (metaEid->nodeNbr == 0 && metaEid->serviceNbr == 0))
	{
		*(metaEid->colon) = ':';
...
static int printCbheEid(char *schemeName, CbheEid *eid, char **result)
{
	...
	eidString = (char *) MTAKE(eidLength);
	if (eidString == NULL)
	{
	...
	{
		isprintf(eidString, eidLength, "%s:" UVAST_FIELDSPEC ".%u", schemeName, eid->nodeNbr, eid->serviceNbr);
	}
...
static int printDtnEid(DtnEid *eid, char *dictionary, char **result)
{
	...
	isprintf(eidString, eidLength, "%s:%s", dictionary + eid->schemeNameOffset, dictionary + eid->nssOffset);
	*result = eidString;
	return 0;
}
...
int startBpTask(Object cmd, Object cmdParms, int *pid)
{
	...
	else
	{
		...
		isprintf(buffer, sizeof buffer, "%s %s", cmdString, parmsString);
	}
...
static void reportStateStats(int i, char *fromTimestamp, char *toTimestamp, unsigned int count_0, uvast bytes_0, unsigned int count_1, uvast bytes_1, unsigned int count_2, uvast bytes_2, unsigned int count_total, uvast bytes_total)
{
	char buffer[256];
...
%u " UVAST_FIELDSPEC " (+) %u " UVAST_FIELDSPEC, classnames[i], fromTimestamp, toTimestamp, count_0, bytes_0, count_1, bytes_1, count_2, bytes_2, count_total, bytes_total);
	writeMemo(buffer);
}
...
void reportAllStateStats()
{
	...
	sdr_read(sdr, (char *) &sourceStats, bpdb.sourceStats, sizeof(BpCosStats));
	reportStateStats(0, fromTimestamp, toTimestamp, sourceStats.tallies[0].currentCount, sourceStats.tallies[0].currentBytes, sourceStats.tallies[1].currentCount, sourceStats.tallies[1].currentBytes, sourceStats.tallies[2].currentCount, sourceStats.tallies[2].currentBytes,
			sourceStats.tallies[0].currentCount + sourceStats.tallies[1].currentCount + sourceStats.tallies[2].currentCount, sourceStats.tallies[0].currentBytes + sourceStats.tallies[1].currentBytes + sourceStats.tallies[2].currentBytes);

	/*	Forwarded.						*/

	sdr_read(sdr, (char *) &dbStats, bpdb.dbStats, sizeof(BpDbStats));
	reportStateStats(1, fromTimestamp, toTimestamp, 0, 0, 0, 0, 0, 0, dbStats.tallies[BP_DB_FWD_OKAY].currentCount, dbStats.tallies[BP_DB_FWD_OKAY].currentBytes);

	/*	Transmitted.						*/

	sdr_read(sdr, (char *) &xmitStats, bpdb.xmitStats, sizeof(BpCosStats));
	reportStateStats(2, fromTimestamp, toTimestamp, xmitStats.tallies[0].currentCount, xmitStats.tallies[0].currentBytes, xmitStats.tallies[1].currentCount, xmitStats.tallies[1].currentBytes, xmitStats.tallies[2].currentCount, xmitStats.tallies[2].currentBytes,
			xmitStats.tallies[0].currentCount + xmitStats.tallies[1].currentCount + xmitStats.tallies[2].currentCount, xmitStats.tallies[0].currentBytes + xmitStats.tallies[1].currentBytes + xmitStats.tallies[2].currentBytes);

	/*	Received.						*/

	sdr_read(sdr, (char *) &recvStats, bpdb.recvStats, sizeof(BpCosStats));
	reportStateStats(3, fromTimestamp, toTimestamp, recvStats.tallies[0].currentCount, recvStats.tallies[0].currentBytes, recvStats.tallies[1].currentCount, recvStats.tallies[1].currentBytes, recvStats.tallies[2].currentCount, recvStats.tallies[2].currentBytes,
			recvStats.tallies[0].currentCount + recvStats.tallies[1].currentCount + recvStats.tallies[2].currentCount, recvStats.tallies[0].currentBytes + recvStats.tallies[1].currentBytes + recvStats.tallies[2].currentBytes);
	...
	reportStateStats(6, fromTimestamp, toTimestamp, 0, 0, 0, 0, 0, 0, dbStats.tallies[BP_DB_REQUEUED_FOR_FWD].currentCount, dbStats.tallies[BP_DB_REQUEUED_FOR_FWD].currentBytes);

	/*	Expired.						*/

	reportStateStats(7, fromTimestamp, toTimestamp, 0, 0, 0, 0, 0, 0, dbStats.tallies[BP_DB_EXPIRED].currentCount, dbStats.tallies[BP_DB_EXPIRED].currentBytes);
	sdr_exit_xn(sdr);
}
...
static int destroyIncomplete(IncompleteBundle *incomplete, Object incElt)
{
	...
	for (elt = sdr_list_first(bpSdr, incomplete->fragments); elt; elt = nextElt)
	{
...
void removeBundleFromQueue(Bundle *bundle, Object bundleObj, ClProtocol *protocol, Object outductObj, Outduct *outduct)
{
	Sdr bpSdr = getIonsdr();
...
static void purgeDuctXmitElt(Bundle *bundle, Object bundleObj)
{
	...
	sdr_read(bpSdr, (char *) &protocol, outduct.protocol, sizeof(ClProtocol));
	removeBundleFromQueue(bundle, bundleObj, &protocol, outductObj, &outduct);
}
...
void destroyBpTimelineEvent(Object timelineElt)
{
	...
	sm_rbt_delete(getIonwm(), (getBpVdb())->timeline, orderBpEvents, &event, NULL, NULL);
...
int bpDestroyBundle(Object bundleObj, int ttlExpired)
{
	...
	if (ttlExpired)
	{
		...
		if (bundle.fragmentElt)
		{
			...
			GET_OBJ_POINTER(bpSdr, IncompleteBundle, incomplete, sdr_list_data(bpSdr, bundle.incompleteElt));
			if (sdr_list_length(bpSdr, incomplete->fragments) == 0)
			{
				if (destroyIncomplete(incomplete, bundle.incompleteElt) < 0)
				{
...
incomplete bundle.", NULL);
		...
		if (!(bundle.bundleProcFlags & BDL_IS_ADMIN) && (bundle.custodyTaken || (SRR_FLAGS(bundle.bundleProcFlags) & BP_DELETED_RPT)))
		{
	...
	if (bundle.fragmentElt || bundle.dlvQueueElt || bundle.fwdQueueElt || bundle.ductXmitElt || bundle.custodyTaken || bundle.detained)
	{
	...
	if (bundle.hashEntry)
	{
		bsetObj = sdr_hash_entry_value(bpSdr, (_bpConstants())->bundles, bundle.hashEntry);
		...
		{
			sdr_write(bpSdr, bsetObj, (char * ) &bset, sizeof(BundleSet));
		}
	}
	...
	if (bundle.trackingElts)
	{
		while (1)
		{
			...
			sdr_list_delete(bpSdr, sdr_list_data(bpSdr, elt), NULL, NULL);
	...
	bpDiscardTally(COS_FLAGS(bundle.bundleProcFlags) & 0x03, bundle.payload.length);
...
static int constructBundleHashKey(char *buffer, char *sourceEid, unsigned int seconds, unsigned int count, unsigned int offset, unsigned int length)
{
	memset(buffer, 0, BUNDLES_HASH_KEY_BUFLEN);
	isprintf(buffer, BUNDLES_HASH_KEY_BUFLEN, "%s:%u:%u:%u:%u", sourceEid, seconds, count, offset, length);
	return strlen(buffer);
}

int findBundle(char *sourceEid, BpTimestamp *creationTime, unsigned int fragmentOffset, unsigned int fragmentLength,
Object *bundleAddr)
{
	Sdr bpSdr = getIonsdr();
	...
	*bundleAddr = 0; /*	Default: not found.		*/
	CHKERR(ionLocked());
	if (constructBundleHashKey(key, sourceEid, creationTime->seconds, creationTime->count, fragmentOffset, fragmentLength) > BUNDLES_HASH_KEY_LEN)
	{
		return 0; /*	Can't be in hash table.		*/
	}

	switch (sdr_hash_retrieve(bpSdr, (_bpConstants())->bundles, key, &bsetObj, &hashElt))
	{
	case -1:
...
void findScheme(char *schemeName, VScheme **scheme, PsmAddress *schemeElt)
{
	...
	for (elt = sm_list_first(bpwm, (_bpvdb(NULL))->schemes); elt; elt = sm_list_next(bpwm, elt))
	{
...
too long", fwdCmd);
	...
	if (admAppCmd)
	{
		...
		{
			if (istrlen(admAppCmd, MAX_SDRSTRING + 1) > MAX_SDRSTRING)
			{
...
too long", admAppCmd);
	...
	if (addr)
	{
		...
		schemeElt = sdr_list_insert_last(bpSdr, (_bpConstants())->schemes, addr);
	}
...
int removeScheme(char *schemeName)
{
	...
	{
		sdr_exit_xn(bpSdr);
		writeMemoNote("[?] Scheme has backlog, can't be removed", schemeName);
		return 0;
	}
	...
	{
		sdr_exit_xn(bpSdr);
		writeMemoNote("[?] Scheme has endpoints, can't be removed", schemeName);
		return 0;
	}
...
void findEndpoint(char *schemeName, char *nss, VScheme *vscheme, VEndpoint **vpoint, PsmAddress *vpointElt)
{
	PsmPartition bpwm = getIonwm();
	PsmAddress elt;
...
int addEndpoint(char *eid, BpRecvRule recvRule, char *script)
{
	...
	if (endpointBuf.stats)
	{
		...
		sdr_write(bpSdr, endpointBuf.stats, (char * ) &statsInit, sizeof(EndpointStats));
	}
	...
	if (addr)
	{
		sdr_read(bpSdr, (char *) &scheme, endpointBuf.scheme, sizeof(Scheme));
		endpointElt = sdr_list_insert_last(bpSdr, scheme.endpoints, addr);
...
int removeEndpoint(char *eid)
{
	...
	{
		sdr_exit_xn(bpSdr);
		writeMemoNote("[?] Endpoint has incomplete bundles pending", eid);
		return 0;
	}
...
void fetchProtocol(char *protocolName, ClProtocol *clp, Object *clpElt)
{
	...
	for (elt = sdr_list_first(bpSdr, (_bpConstants())->protocols); elt; elt = sdr_list_next(bpSdr, elt))
	{
		sdr_read(bpSdr, (char *) clp, sdr_list_data(bpSdr, elt), sizeof(ClProtocol));
...
int addProtocol(char *protocolName, int payloadPerFrame, int ohdPerFrame, int nominalRate, int protocolClass)
{
	Sdr bpSdr = getIonsdr();
	ClProtocol clpbuf;
	Object elt;
	...
	CHKERR(protocolName);
	if (*protocolName == 0 || strlen(protocolName) > MAX_CL_PROTOCOL_NAME_LEN)
	{
	...
	{
		writeMemoNote("[?] Per-frame payload and overhead must be > 0", protocolName);
		return 0;
	}
...
int removeProtocol(char *protocolName)
{
	...
	if (sdr_list_length(bpSdr, clpbuf.inducts) != 0 || sdr_list_length(bpSdr, clpbuf.outducts) != 0)
	{
		sdr_exit_xn(bpSdr);
		writeMemoNote("[?] Protocol has ducts, can't be removed", protocolName);
		return 0;
	}
...
void findInduct(char *protocolName, char *ductName, VInduct **vduct, PsmAddress *vductElt)
{
	PsmPartition bpwm = getIonwm();
	PsmAddress elt;

	for (elt = sm_list_first(bpwm, (_bpvdb(NULL))->inducts); elt; elt = sm_list_next(bpwm, elt))
	{
		*vduct = (VInduct *) psp(bpwm, sm_list_data(bpwm, elt));
		if (strcmp((*vduct)->protocolName, protocolName) == 0 && strcmp((*vduct)->ductName, ductName) == 0)
		{
			break;
		}
	}
...
int addInduct(char *protocolName, char *ductName, char *cliCmd)
{
	...
	if (cliCmd)
	{
		...
		else
		{
			...
			{
				writeMemoNote("[?] CLI command string too long", cliCmd);
				return 0;
			}
		}
	}
	...
	if (ductBuf.stats)
	{
		...
		sdr_write(bpSdr, ductBuf.stats, (char * ) &statsInit, sizeof(InductStats));
	}
...
void findOutduct(char *protocolName, char *ductName, VOutduct **vduct, PsmAddress *vductElt)
{
	PsmPartition bpwm = getIonwm();
	PsmAddress elt;

	for (elt = sm_list_first(bpwm, (_bpvdb(NULL))->outducts); elt; elt = sm_list_next(bpwm, elt))
	{
...
int maxPayloadLengthKnown(VOutduct *vduct, unsigned int *maxPayloadLength)
{
	...
	*maxPayloadLength = 0; /*	Default: unlimited.	*/
	if (vduct->neighborNodeNbr) /*	Known neighbor node.	*/
	{
		...
		rfx_contact_state(vduct->neighborNodeNbr, &secRemaining, &xmitRate);
		sdr_exit_xn(sdr);
...
int addOutduct(char *protocolName, char *ductName, char *cloCmd, unsigned int maxPayloadLength)
{
	Sdr bpSdr = getIonsdr();
	ClProtocol clpbuf;
	Object clpElt;
	...
	if (cloCmd)
	{
		...
		else
		{
			...
			{
				writeMemoNote("[?] CLO command string too long", cloCmd);
				return 0;
			}
		}
	}
	...
	if (ductBuf.stats)
	{
		...
		sdr_write(bpSdr, ductBuf.stats, (char * ) &statsInit, sizeof(OutductStats));
	}
...
int updateOutduct(char *protocolName, char *ductName, char *cloCmd, unsigned int maxPayloadLength)
{
	Sdr bpSdr = getIonsdr();
	VOutduct *vduct;
	PsmAddress vductElt;
...
int removeOutduct(char *protocolName, char *ductName)
{
	...
	if (sdr_list_length(bpSdr, outductBuf.bulkQueue) != 0 || sdr_list_length(bpSdr, outductBuf.stdQueue) != 0 || sdr_list_length(bpSdr, outductBuf.urgentQueue) != 0)
	{
		sdr_exit_xn(bpSdr);
...
Object *incompleteAddr, Object *incompleteElt)
{
	...
	*incompleteElt = 0;
	GET_OBJ_POINTER(bpSdr, Endpoint, endpoint, sdr_list_data(bpSdr, vpoint->endpointElt));
	for (elt = sdr_list_first(bpSdr, endpoint->incompletes); elt; elt = sdr_list_next(bpSdr, elt))
	{
		*incompleteAddr = sdr_list_data(bpSdr, elt);
		GET_OBJ_POINTER(bpSdr, IncompleteBundle, incomplete, *incompleteAddr);

		/*	See if ID of Incomplete's first fragment
		 *	matches ID of the bundle we're looking for.	*/

		GET_OBJ_POINTER(bpSdr, Bundle, fragment, sdr_list_data(bpSdr, sdr_list_first(bpSdr, incomplete->fragments)));
		...
		{
			if (fragment->id.source.c.nodeNbr != bundle->id.source.c.nodeNbr || fragment->id.source.c.serviceNbr != bundle->id.source.c.serviceNbr)
			{
				continue;
			}
		}
		...
		{
			if ((fragDictionary = retrieveDictionary(fragment)) == (char *) fragment)
			{
			...
			result = (strcmp(fragDictionary + fragment->id.source.d.schemeNameOffset, argDictionary + bundle->id.source.d.schemeNameOffset) != 0) || (strcmp(fragDictionary + fragment->id.source.d.nssOffset, argDictionary + bundle->id.source.d.nssOffset) != 0);
		...
		if (fragment->id.creationTime.seconds == bundle->id.creationTime.seconds && fragment->id.creationTime.count == bundle->id.creationTime.count)
		{
...
static void computeExpirationTime(Bundle *bundle)
{
	...
	{
		bundle->expirationTime = bundle->id.creationTime.seconds + bundle->timeToLive;
	}
	else /*	Round remaining time to nearest second.		*/
	{
		...
		expirationTime.tv_sec = bundle->arrivalTime.tv_sec + timeRemaining.tv_sec;
		expirationTime.tv_usec = bundle->arrivalTime.tv_usec + timeRemaining.tv_usec;
...
static int catalogueBundle(Bundle *bundle, Object bundleObj)
{
	...
	if (constructBundleHashKey(bundleKey, sourceEid, bundle->id.creationTime.seconds, bundle->id.creationTime.count, bundle->id.fragmentOffset, bundle->totalAduLength == 0 ? 0 : bundle->payload.length) > BUNDLES_HASH_KEY_LEN)
	{
...
int bpClone(Bundle *oldBundle, Bundle *newBundle, Object *newBundleObj, unsigned int offset, unsigned int length)
{
	Sdr bpSdr = getIonsdr();
	...
	if (length == 0 || (offset + length) > oldBundle->payload.length)
	{
	...
	*newBundleObj = sdr_malloc(bpSdr, sizeof(Bundle));
	...
	if (oldBundle->dictionary) /*	Must copy dictionary.	*/
	{
		...
		newBundle->dictionary = sdr_malloc(bpSdr, oldBundle->dictionaryLength);
		...
		sdr_write(bpSdr, newBundle->dictionary, dictionaryBuffer, oldBundle->dictionaryLength);
		releaseDictionary(dictionaryBuffer);
	}

	/*	Clone part or all of payload.				*/

	newBundle->payload.content = zco_clone(bpSdr, oldBundle->payload.content, offset, length);
	...
	if (length != oldBundle->payload.length)
	{
		...
		{
			newBundle->id.fragmentOffset = oldBundle->id.fragmentOffset + offset;
	...
	{
		result = putBpString(&(newBundle->clDossier.senderEid), senderEid);
		MRELEASE(senderEid);
		if (result < 0)
		{
			putErrmsg("Can't copy sender EID.", NULL);
	...
	{
		newBundle->proxNodeEid = sdr_string_dup(bpSdr, oldBundle->proxNodeEid);
	...
	{
		newBundle->destDuctName = sdr_string_dup(bpSdr, oldBundle->destDuctName);
...
int forwardBundle(Object bundleObj, Bundle *bundle, char *eid)
{
	...
	for (elt = sdr_list_first(bpSdr, bundle->stations); elt; elt = sdr_list_next(bpSdr, elt))
	{
	...
	if (stationEid == 0 || sdr_list_insert_first(bpSdr, bundle->stations, stationEid) == 0)
	{
	...
	sdr_read(bpSdr, (char *) &schemeBuf, sdr_list_data(bpSdr, vscheme->schemeElt), sizeof(Scheme));
	bundle->fwdQueueElt = sdr_list_insert_last(bpSdr, schemeBuf.forwardQueue, bundleObj);
	sdr_write(bpSdr, bundleObj, (char * ) bundle, sizeof(Bundle));
...
static void loadCbheEids(Bundle *bundle, MetaEid *destMetaEid, MetaEid *sourceMetaEid, MetaEid *reportToMetaEid)
{
	/*	Destination.						*/
	...
	if (reportToMetaEid == NULL)
	{
		if (sourceMetaEid) /*	Default to source.	*/
		{
			...
			bundle->reportTo.c.serviceNbr = sourceMetaEid->serviceNbr;
		}
		else /*	None.			*/
		{
...
static int addStringToDictionary(char **strings, int *stringLengths, int *stringCount, unsigned int *dictionaryLength, char *string, int stringLength)
{
	int offset = 0;
	int i;
...
static char *loadDtnEids(Bundle *bundle, MetaEid *destMetaEid, MetaEid *sourceMetaEid, MetaEid *reportToMetaEid)
{
	char *strings[8];
	int stringLengths[8];
	int stringCount = 0;
	...
	bundle->custodian.unicast = 1;
	bundle->custodian.d.schemeNameOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, "dtn", 3);
	bundle->custodian.d.nssOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, "none", 4);

	/*	Destination.						*/

	bundle->destination.cbhe = 0;
	bundle->destination.unicast = 1;
	bundle->destination.d.schemeNameOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, destMetaEid->schemeName, destMetaEid->schemeNameLength);
	bundle->destination.d.nssOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, destMetaEid->nss, destMetaEid->nssLength);
	...
	if (sourceMetaEid == NULL) /*	Anonymous.		*/
	{
		...
		bundle->id.source.d.schemeNameOffset = bundle->custodian.d.schemeNameOffset;
		bundle->id.source.d.nssOffset = bundle->custodian.d.nssOffset;
	}
	else
	{
		bundle->id.source.d.schemeNameOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, sourceMetaEid->schemeName, sourceMetaEid->schemeNameLength);
		bundle->id.source.d.nssOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, sourceMetaEid->nss, sourceMetaEid->nssLength);
	}
	...
	if (reportToMetaEid == NULL)
	{
		if (sourceMetaEid) /*	Default to source.	*/
		{
			bundle->reportTo.d.schemeNameOffset = bundle->id.source.d.schemeNameOffset;
			bundle->reportTo.d.nssOffset = bundle->id.source.d.nssOffset;
		}
		else /*	None.			*/
		{
			/*	Custodian is known to be "none".	*/

			bundle->reportTo.d.schemeNameOffset = bundle->custodian.d.schemeNameOffset;
			bundle->reportTo.d.nssOffset = bundle->custodian.d.nssOffset;
		}
	}
	else
	{
		bundle->reportTo.d.schemeNameOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, reportToMetaEid->schemeName, reportToMetaEid->schemeNameLength);
		bundle->reportTo.d.nssOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, reportToMetaEid->nss, reportToMetaEid->nssLength);
	}
...
int bpSend(MetaEid *sourceMetaEid, char *destEidString, char *reportToEidString, int lifespan, int classOfService, BpCustodySwitch custodySwitch, unsigned char srrFlagsByte, int ackRequested, BpExtendedCOS *extendedCOS, Object adu,
Object *bundleObj, int adminRecordType)
{
	...
	if (parseEidString(destEidString, &destMetaEid, &vscheme, &vschemeElt) == 0)
	{
	...
	if (neighborElt)
	{
		neighbor = (NdpNeighbor *) psp(bpwm, sm_list_data(bpwm, neighborElt));
...
when asking for custody transfer and/or status reports.");
	...
	if (adminRecordType != 0)
	{
		...
		if (sourceMetaEid == NULL)
		{
			istrcpy(sourceEidString, vscheme->adminEid, sizeof sourceEidString);
			if (parseEidString(sourceEidString, &tempMetaEid, &vscheme2, &vschemeElt2) == 0)
			{
				restoreEidString(&tempMetaEid);
				writeMemoNote("[?] Admin EID malformed", sourceEidString);
				return 0;
			}
	...
	{
		if (parseEidString(reportToEidString, &reportToMetaEidBuf, &vscheme, &vschemeElt) == 0)
		{
			restoreEidString(&reportToMetaEidBuf);
			writeMemoNote("[?] Report-to EID malformed", reportToEidString);
			return 0;
		}
	...
	{
		dictionary = NULL;
		loadCbheEids(&bundle, &destMetaEid, sourceMetaEid, reportToMetaEid);
	}
	else /*	Not all endpoints are in same CBHE scheme.	*/
	{
		dictionary = loadDtnEids(&bundle, &destMetaEid, sourceMetaEid, reportToMetaEid);
		if (dictionary == NULL)
		{
			sdr_exit_xn(bpSdr);
			restoreEidString(&destMetaEid);
	...
	if (dictionary)
	{
		...
		sdr_write(bpSdr, bundle.dictionary, dictionary, bundle.dictionaryLength);
		MRELEASE(dictionary);
	}
	...
	if (bundleAddr == 0 || bundle.stations == 0 || bundle.trackingElts == 0 || bundle.extensions[0] == 0 || bundle.extensions[1] == 0)
	{
	...
	if (extendedCOS)
	{
		...
		memcpy(bundle.extendedCOS.metadata, extendedCOS->metadata, sizeof bundle.extendedCOS.metadata);
	}
	...
	for (i = 0, spec = extensions; i < extensionsCt; i++, spec++)
	{
		...
		if (def != NULL && def->offer != NULL)
		{
			...
			{
				putErrmsg("Failed offering extension block.", NULL);
				sdr_cancel_xn(bpSdr);
				return -1;
			}
	...
	if (vpoint)
	{
		bpEndpointTally(vpoint, BP_ENDPOINT_SOURCED, bundle.payload.length);
	}
...
static int sendCtSignal(Bundle *bundle, char *dictionary, int succeeded, BpCtReason reasonCode)
{
	char *custodianEid;
	...
	{
		bundle->ctSignal.creationTime.seconds = bundle->id.creationTime.seconds;
		bundle->ctSignal.creationTime.count = bundle->id.creationTime.count;
	}
	...
	if (ttl < 1)
		ttl = 1;
	if (printEid(&bundle->id.source, dictionary, &bundle->ctSignal.sourceEid) < 0)
	{
		putErrmsg("Can't print source EID.", NULL);
		MRELEASE(custodianEid);
		return -1;
	...
	result = bpSend(NULL, custodianEid, NULL, ttl, BP_EXPEDITED_PRIORITY, NoCustodyRequested, 0, 0, &ecos, payloadZco, NULL,
	BP_CUSTODY_SIGNAL);
	MRELEASE(custodianEid);
	switch (result)
	{
...
static int noteCtEvent(Bundle *bundle, AcqWorkArea *work, char *dictionary, int succeeded, BpCtReason reasonCode)
{
...
int sendStatusRpt(Bundle *bundle, char *dictionary)
{
	...
	unsigned int ttl; /*	Original bundle's TTL.		*/
	BpExtendedCOS ecos =
	{ 0, 0, bundle->extendedCOS.ordinal };
	Object payloadZco = 0;
	char *reportToEid;
	int result;
	...
	{
		bundle->statusRpt.creationTime.seconds = bundle->id.creationTime.seconds;
		bundle->statusRpt.creationTime.count = bundle->id.creationTime.count;
	}
	...
	if (ttl < 1)
		ttl = 1;
	if (printEid(&bundle->id.source, dictionary, &bundle->statusRpt.sourceEid) < 0)
	{
		putErrmsg("Can't print source EID.", NULL);
		return -1;
	}
	...
	result = bpSend(NULL, reportToEid, NULL, ttl, priority, NoCustodyRequested, 0, 0, &ecos, payloadZco, NULL,
	BP_STATUS_REPORT);
	...
	bundle->statusRpt.reasonCode = (BpSrReason) 0;
...
void lookUpEndpoint(EndpointId eid, char *dictionary, VScheme *vscheme, VEndpoint **vpoint)
{
	PsmPartition bpwm = getIonwm();
	char nssBuf[42];
	char *nss;
	...
	if (dictionary == NULL)
	{
		isprintf(nssBuf, sizeof nssBuf, UVAST_FIELDSPEC ".%u", eid.c.nodeNbr, eid.c.serviceNbr);
		nss = nssBuf;
	}
	else
	{
...
static int enqueueForDelivery(Object bundleObj, Bundle *bundle, VEndpoint *vpoint)
{
	Sdr bpSdr = getIonsdr();
	OBJ_POINTER(Endpoint, endpoint);
	...
	if (vpoint->appPid == ERROR) /*	Not open by any app.	*/
	{
		...
		{
			if (sdr_string_read(bpSdr, scriptBuf, endpoint->recvScript) < 0)
			{
		...
		{
			sdr_write(bpSdr, bundleObj, (char * ) bundle, sizeof(Bundle));
			bpEndpointTally(vpoint, BP_ENDPOINT_ABANDONED, bundle->payload.length);
	...
	bundle->dlvQueueElt = sdr_list_insert_last(bpSdr, endpoint->deliveryQueue, bundleObj);
...
Object bundleObj, Bundle *bundle)
{
	...
	for (elt = sdr_list_first(bpSdr, incomplete->fragments); elt; elt = sdr_list_next(bpSdr, elt))
	{
		GET_OBJ_POINTER(bpSdr, Bundle, fragment, sdr_list_data(bpSdr, elt));
	...
	if (elt)
	{
		bundle->fragmentElt = sdr_list_insert_before(bpSdr, elt, bundleObj);
	}
	else
	{
		bundle->fragmentElt = sdr_list_insert_last(bpSdr, incomplete->fragments, bundleObj);
	}
...
static int createIncompleteBundle(Object bundleObj, Bundle *bundle, VEndpoint *vpoint)
{
	Sdr bpSdr = getIonsdr();
	...
	sdr_write(bpSdr, incObj, (char * ) &incomplete, sizeof(IncompleteBundle));
	GET_OBJ_POINTER(bpSdr, Endpoint, endpoint, sdr_list_data(bpSdr, vpoint->endpointElt));
	bundle->incompleteElt = sdr_list_insert_last(bpSdr, endpoint->incompletes, incObj);
	...
	sdr_list_user_data_set(bpSdr, incomplete.fragments, bundle->incompleteElt);
...
int deliverBundle(Object bundleObj, Bundle *bundle, VEndpoint *vpoint)
{
	...
	if (bundleIsCustodial(bundle))
	{
		...
		result = noteCtEvent(bundle, NULL, dictionary, 1, (BpCtReason) 0);
		releaseDictionary(dictionary);
		if (result < 0)
		{
	...
	{
		GET_OBJ_POINTER(getIonsdr(), IncompleteBundle, incomplete, incompleteAddr);
...
static int dispatchBundle(Object bundleObj, Bundle *bundle, VEndpoint **vpoint)
{
	Sdr bpSdr = getIonsdr();
	...
	{
		lookUpEndpoint(bundle->destination, dictionary, vscheme, vpoint);
		...
		{
			if (bundle->destination.cbhe && bundle->destination.unicast && bundle->destination.c.nodeNbr == getOwnNodeNbr())
			{
	...
	{
		bundle->transitElt = sdr_list_insert_last(bpSdr, db->provisionalTransit, bundleObj);
		sm_SemGive(vdb->provisionalTransitSemaphore);
	}
	else
	{
		bundle->transitElt = sdr_list_insert_last(bpSdr, db->confirmedTransit, bundleObj);
...
int bpBeginAcq(AcqWorkArea *work, int authentic, char *senderEid)
{
	...
	if (senderEid)
	{
		...
		work->senderEid = (char *) MTAKE(eidLen);
...
int bpLoadAcq(AcqWorkArea *work, Object zco)
{
	...
	CHKERR(sdr_begin_xn(bpSdr));
	work->zcoElt = sdr_list_insert_last(bpSdr, bpConstants->inboundBundles, zco);
	if (sdr_end_xn(bpSdr) < 0)
	{
...
int bpContinueAcq(AcqWorkArea *work, char *bytes, int length, ReqAttendant *attendant)
{
	...
	if (ionRequestZcoSpace(ZcoInbound, fileSpaceNeeded, 0, heapSpaceNeeded, 0, 0, attendant, &ticket) < 0)
	{
	...
	{
		work->zco = zco_create(sdr, ZcoSdrSource, 0, 0, 0, ZcoInbound, 0);
		...
		work->zcoElt = sdr_list_insert_last(sdr, bpdb->inboundBundles, work->zco);
		if (work->zcoElt == 0)
		{
			putErrmsg("Can't start inbound bundle ZCO.", NULL);
			sdr_cancel_xn(sdr);
	...
	if (source == ZcoSdrSource)
	{
		...
		if (extentObj)
		{
			...
			switch (zco_append_extent(sdr, work->zco, ZcoSdrSource, extentObj, 0, 0 - length))
			{
			case ERROR:
			case 0:
	...
	if (work->acqFileRef == 0) /*	First file extent.	*/
	{
		...
		acqCount++;
		isprintf(fileName, sizeof fileName, "%s%cbpacq.%u", cwd, ION_PATH_DELIMITER, acqCount);
		...
		fileLength = 0;
		work->acqFileRef = zco_create_file_ref(sdr, fileName, "", ZcoInbound);
	...
	{
		oK(zco_file_ref_path(sdr, work->acqFileRef, fileName, sizeof fileName));
	...
	switch (zco_append_extent(sdr, work->zco, ZcoFileSource, work->acqFileRef, fileLength, 0 - length))
	{
	case ERROR:
	case 0:
...
// we not take into account protocol overhead
...
static int advanceWorkBuffer(AcqWorkArea *work, int bytesParsed)
{
	...
	if (bytesToReceive > 0)
	{
		bytesReceived = zco_receive_source(getIonsdr(), &(work->reader), bytesToReceive, work->buffer + work->bytesBuffered);
...
static char *_versionMemo()
{
	...
	if (memo == NULL)
	{
		isprintf(buf, sizeof buf, "[?] Wrong bundle version (should be %d)", BP_VERSION);
		memo = buf;
	}
...
static int acquirePrimaryBlock(AcqWorkArea *work)
{
	...
	extractSmallSdnv(&(bundle->id.creationTime.seconds), &cursor, &unparsedBytes);

	extractSmallSdnv(&(bundle->id.creationTime.count), &cursor, &unparsedBytes);
	...
	{
		work->dictionary = (char *) MTAKE(bundle->dictionaryLength);
	...
	if (istrlen(eidString, nullEidLen + 1) == nullEidLen && strcmp(eidString, _nullEid()) == 0)
	{
		bundle->anonymous = 1;
	}
	...
	if (bundle->bundleProcFlags & BDL_IS_FRAGMENT)
	{
		extractSmallSdnv(&(bundle->id.fragmentOffset), &cursor, &unparsedBytes);
		extractSmallSdnv(&(bundle->totalAduLength), &cursor, &unparsedBytes);
	}
	else
	{
		bundle->id.fragmentOffset = 0;
...
static int acquireBlock(AcqWorkArea *work)
{
	...
	if (blkProcFlags & BLK_HAS_EID_REFERENCES)
	{
		...
		{
			extractSmallSdnv(&schemeOffset, &cursor, &unparsedBytes);
			temp = schemeOffset;
			if (lyst_insert_last(eidReferences, (void *) temp) == NULL)
			{
				return -1;
			}
	...
	if (def)
	{
		if (acquireExtensionBlock(work, def, startOfBlock, lengthOfBlock, blkType, blkProcFlags, &eidReferences, dataLength) < 0)
		{
			return -1;
		}
	}
...
static int acqFromWork(AcqWorkArea *work)
{
	...
	if (work->bundle.payload.length <= work->bytesBuffered)
	{
		...
		CHKERR(advanceWorkBuffer(work, work->bundle.payload.length) == 0);
	}
	else
	{
		...
		unreceivedPayload = work->bundle.payload.length - work->bytesBuffered;
		bytesRecd = zco_receive_source(sdr, &(work->reader), unreceivedPayload, NULL);
		CHKERR(bytesRecd >= 0);
...
static int discardReceivedBundle(AcqWorkArea *work, BpCtReason ctReason, BpSrReason srReason)
{
	Bundle *bundle = &(work->bundle);
	Sdr bpSdr;
	...
	if (bundleIsCustodial(bundle))
	{
		...
		if (sdr_end_xn(bpSdr) < 0 || noteCtEvent(bundle, work, work->dictionary, 0, ctReason) < 0)
		{
			putErrmsg("Can't send custody signal.", NULL);
			return -1;
		}
	...
	if (srReason != 0 && (SRR_FLAGS(bundle->bundleProcFlags) & BP_DELETED_RPT))
	{
		bundle->statusRpt.flags |= BP_DELETED_RPT;
...
static int acquireBundle(Sdr bpSdr, AcqWorkArea *work, VEndpoint **vpoint)
{
	...
	if (work->bundleLength > 0)
	{
		...
		work->rawBundle = zco_clone(bpSdr, work->zco, work->zcoBytesConsumed, work->bundleLength);
		if (work->rawBundle <= 0)
		{
	...
	bundle->payload.content = zco_clone(bpSdr, work->rawBundle, work->headerLength, bundle->payload.length);
	...
	if (work->malformed || work->lastBlockParsed == 0)
	{
		...
		bpInductTally(work->vduct, BP_INDUCT_MALFORMED, bundle->payload.length);
		return abortBundleAcq(work);
	}
	...
	if (work->congestive)
	{
		...
		bpInductTally(work->vduct, BP_INDUCT_CONGESTIVE, bundle->payload.length);
		return discardReceivedBundle(work, CtDepletedStorage, SrDepletedStorage);
	}
	...
	if (bundle->clDossier.authentic == 0)
	{
		...
		bpInductTally(work->vduct, BP_INDUCT_INAUTHENTIC, bundle->payload.length);
		return abortBundleAcq(work);
	}
...
//	}
	...
	{
		bpInductTally(work->vduct, BP_INDUCT_MALFORMED, bundle->payload.length);
		return discardReceivedBundle(work, CtBlockUnintelligible, SrBlockUnintelligible);
	}
	...
	if (bundleIsCustodial(bundle))
	{
		if (printEid(&(bundle->custodian), work->dictionary, &eidString) < 0)
		{
		...
		findScheme(custodialSchemeName, &vscheme, &vschemeElt);
		if (vschemeElt != 0 && strcmp(eidString, vscheme->adminEid) == 0)
		{
		...
		else
		{
			...
			count = findBundle(eidString, &(bundle->id.creationTime), bundle->id.fragmentOffset, (bundle->bundleProcFlags & BDL_IS_FRAGMENT ? bundle->payload.length : 0), &bundleAddr);
			MRELEASE(eidString);
			switch (count)
			{
			case -1:
			...
			default: /*	Entry found; redundant.	*/
				return discardReceivedBundle(work, CtRedundantReception, (BpSrReason) 0);
			}
		}
	}
	...
	if (work->senderEid)
	{
		if (parseEidString(work->senderEid, &senderMetaEid, &vscheme, &vschemeElt) == 0)
		{
			restoreEidString(&senderMetaEid);
			writeMemoNote("[?] Sender EID malformed", work->senderEid);
			return abortBundleAcq(work);
		}
	...
	if (bundle->dictionaryLength > 0)
	{
		...
		sdr_write(bpSdr, bundle->dictionary, work->dictionary, bundle->dictionaryLength);
		bundle->dbOverhead += bundle->dictionaryLength;
	}
	...
	bpRecvTally(COS_FLAGS(bundle->bundleProcFlags) & 0x03, bundle->payload.length);
...
static int checkIncompleteBundle(Bundle *newFragment, VEndpoint *vpoint)
{
	...
	for (elt = sdr_list_first(bpSdr, incomplete->fragments); elt; elt = sdr_list_next(bpSdr, elt))
	{
		...
		endOfFragment = fragment->id.fragmentOffset + fragment->payload.length;
	...
	sdr_stage(bpSdr, (char *) &aggregateBundle, aggregateBundleObj, sizeof(Bundle));
	...
	while (1)
	{
		...
		sdr_stage(bpSdr, (char *) &fragBuf, fragmentObj, sizeof(Bundle));
		...
		if (bytesToSkip < fragBuf.payload.length)
		{
			...
			if (zco_clone_source_data(bpSdr, aggregateBundle.payload.content, fragBuf.payload.content, bytesToSkip, bytesToCopy) < 0)
			{
		...
		sdr_write(bpSdr, fragmentObj, (char * ) &fragBuf, sizeof(Bundle));
...
static int constructCtSignal(BpCtSignal *csig, Object *zco)
{
	...
	ctSignalLength = 2 + fragmentOffsetSdnv.length + fragmentLengthSdnv.length + signalTimeSecondsSdnv.length + signalTimeNanosecSdnv.length + creationTimeSecondsSdnv.length + creationTimeCountSdnv.length + eidLengthSdnv.length + eidLength;
	buffer = (char *) MTAKE(ctSignalLength);
	if (buffer == NULL)
	{
	...
	*cursor = csig->reasonCode | (csig->succeeded << 7);
	...
	memcpy(cursor, signalTimeSecondsSdnv.text, signalTimeSecondsSdnv.length);
	cursor += signalTimeSecondsSdnv.length;

	memcpy(cursor, signalTimeNanosecSdnv.text, signalTimeNanosecSdnv.length);
	cursor += signalTimeNanosecSdnv.length;

	memcpy(cursor, creationTimeSecondsSdnv.text, creationTimeSecondsSdnv.length);
	cursor += creationTimeSecondsSdnv.length;

	memcpy(cursor, creationTimeCountSdnv.text, creationTimeCountSdnv.length);
	...
	*zco = zco_create(bpSdr, ZcoSdrSource, sourceData, 0, 0 - ctSignalLength, ZcoOutbound, 0);
...
static int parseCtSignal(BpCtSignal *csig, unsigned char *cursor, int unparsedBytes, int isFragment)
{
	unsigned char head1;
	...
	csig->reasonCode = (BpCtReason) (head1 & 0x7f);

	if (isFragment)
	{
		extractSmallSdnv(&(csig->fragmentOffset), &cursor, &unparsedBytes);
		extractSmallSdnv(&(csig->fragmentLength), &cursor, &unparsedBytes);
	}
	...
	extractSmallSdnv(&(csig->creationTime.seconds), &cursor, &unparsedBytes);
	...
	csig->sourceEid = (char *) MTAKE(eidLength + 1);
...
static int constructStatusRpt(BpStatusRpt *rpt, Object *zco)
{
	...
	rptLength = 3 + fragmentOffsetSdnv.length + fragmentLengthSdnv.length + receiptTimeSecondsSdnv.length + receiptTimeNanosecSdnv.length + custodyTimeSecondsSdnv.length + custodyTimeNanosecSdnv.length + forwardTimeSecondsSdnv.length + forwardTimeNanosecSdnv.length + deliveryTimeSecondsSdnv.length
			+ deliveryTimeNanosecSdnv.length + deletionTimeSecondsSdnv.length + deletionTimeNanosecSdnv.length + creationTimeSecondsSdnv.length + creationTimeCountSdnv.length + eidLengthSdnv.length + eidLength;
	buffer = (char *) MTAKE(rptLength);
	if (buffer == NULL)
	{
		putErrmsg("Can't construct status report.", NULL);
		return -1;
	...
	*cursor = rpt->reasonCode;
	...
	memcpy(cursor, receiptTimeSecondsSdnv.text, receiptTimeSecondsSdnv.length);
	cursor += receiptTimeSecondsSdnv.length;

	memcpy(cursor, receiptTimeNanosecSdnv.text, receiptTimeNanosecSdnv.length);
	cursor += receiptTimeNanosecSdnv.length;

	memcpy(cursor, custodyTimeSecondsSdnv.text, custodyTimeSecondsSdnv.length);
	cursor += custodyTimeSecondsSdnv.length;

	memcpy(cursor, custodyTimeNanosecSdnv.text, custodyTimeNanosecSdnv.length);
	cursor += custodyTimeNanosecSdnv.length;

	memcpy(cursor, forwardTimeSecondsSdnv.text, forwardTimeSecondsSdnv.length);
	cursor += forwardTimeSecondsSdnv.length;

	memcpy(cursor, forwardTimeNanosecSdnv.text, forwardTimeNanosecSdnv.length);
	cursor += forwardTimeNanosecSdnv.length;

	memcpy(cursor, deliveryTimeSecondsSdnv.text, deliveryTimeSecondsSdnv.length);
	cursor += deliveryTimeSecondsSdnv.length;

	memcpy(cursor, deliveryTimeNanosecSdnv.text, deliveryTimeNanosecSdnv.length);
	cursor += deliveryTimeNanosecSdnv.length;

	memcpy(cursor, deletionTimeSecondsSdnv.text, deletionTimeSecondsSdnv.length);
	cursor += deletionTimeSecondsSdnv.length;

	memcpy(cursor, deletionTimeNanosecSdnv.text, deletionTimeNanosecSdnv.length);
	...
	*zco = zco_create(bpSdr, ZcoSdrSource, sourceData, 0, 0 - rptLength, ZcoOutbound, 0);
	if (sdr_end_xn(bpSdr) < 0 || *zco == (Object) ERROR || *zco == 0)
	{
...
static int parseStatusRpt(BpStatusRpt *rpt, unsigned char *cursor, int unparsedBytes, int isFragment)
{
	unsigned int eidLength;
	...
	rpt->flags = *cursor;
	cursor++;
	rpt->reasonCode = (BpSrReason) *cursor;
	cursor++;
	unparsedBytes -= 2;
	...
	rpt->sourceEid = (char *) MTAKE(eidLength + 1);
...
static int parseAdminRecord(int *adminRecordType, BpStatusRpt *rpt, BpCtSignal *csig, void **otherPtr, Object payload)
{
	Sdr bpSdr = getIonsdr();
	unsigned int buflen;
	char *buffer;
	...
	if ((buffer = (char *) MTAKE(buflen)) == NULL)
	{
	...
	*adminRecordType = (*cursor >> 4) & 0x0f;
	...
	case BP_STATUS_REPORT:
		result = parseStatusRpt(rpt, (unsigned char *) cursor, unparsedBytes, bundleIsFragment);
		break;

	case BP_CUSTODY_SIGNAL:
		result = parseCtSignal(csig, (unsigned char *) cursor, unparsedBytes, bundleIsFragment);
		break;
	...
	default: /*	Unknown or non-standard admin record.	*/
		result = parseACS(*adminRecordType, otherPtr, (unsigned char *) cursor, unparsedBytes, bundleIsFragment);
		if (result != -2) /*	Parsed the record.	*/
		{
			break;
		}

		result = parseImcPetition(*adminRecordType, otherPtr, (unsigned char *) cursor, unparsedBytes);
		if (result != -2) /*	Parsed the record.	*/
		{
			break;
		}
...
static int catenateBundle(Bundle *bundle)
{
	...
	{
		encodeSdnv(&(eidSdnvs[0]), bundle->destination.d.schemeNameOffset);
		totalLengthOfEidSdnvs += eidSdnvs[0].length;
		encodeSdnv(&(eidSdnvs[1]), bundle->destination.d.nssOffset);
		totalLengthOfEidSdnvs += eidSdnvs[1].length;
		encodeSdnv(&(eidSdnvs[2]), bundle->id.source.d.schemeNameOffset);
		totalLengthOfEidSdnvs += eidSdnvs[2].length;
		encodeSdnv(&(eidSdnvs[3]), bundle->id.source.d.nssOffset);
		totalLengthOfEidSdnvs += eidSdnvs[3].length;
		encodeSdnv(&(eidSdnvs[4]), bundle->reportTo.d.schemeNameOffset);
		totalLengthOfEidSdnvs += eidSdnvs[4].length;
		encodeSdnv(&(eidSdnvs[5]), bundle->reportTo.d.nssOffset);
		totalLengthOfEidSdnvs += eidSdnvs[5].length;
		encodeSdnv(&(eidSdnvs[6]), bundle->custodian.d.schemeNameOffset);
		totalLengthOfEidSdnvs += eidSdnvs[6].length;
		encodeSdnv(&(eidSdnvs[7]), bundle->custodian.d.nssOffset);
		totalLengthOfEidSdnvs += eidSdnvs[7].length;
	}
	...
	residualBlkLength = totalLengthOfEidSdnvs + creationTimestampTimeSdnv.length + creationTimestampCountSdnv.length + lifetimeSdnv.length + dictionaryLengthSdnv.length + bundle->dictionaryLength + fragmentOffsetSdnv.length + totalAduLengthSdnv.length;
	...
	totalHeaderLength = 1 + bundleProcFlagsSdnv.length + residualBlkLengthSdnv.length + residualBlkLength + bundle->extensionsLength[PRE_PAYLOAD] + 1 + blkProcFlagsSdnv.length + payloadLengthSdnv.length;
	buffer = (unsigned char *) MTAKE(totalHeaderLength);
	if (buffer == NULL)
	{
	...
	*cursor = BP_VERSION;
	...
	memcpy(cursor, residualBlkLengthSdnv.text, residualBlkLengthSdnv.length);
	...
	memcpy(cursor, creationTimestampTimeSdnv.text, creationTimestampTimeSdnv.length);
	cursor += creationTimestampTimeSdnv.length;

	memcpy(cursor, creationTimestampCountSdnv.text, creationTimestampCountSdnv.length);
	...
	if (bundle->dictionaryLength > 0)
	{
		sdr_read(bpSdr, (char *) cursor, bundle->dictionary, bundle->dictionaryLength);
		cursor += bundle->dictionaryLength;
	}
	...
	if (bundle->bundleProcFlags & BDL_IS_FRAGMENT)
	{
		memcpy(cursor, fragmentOffsetSdnv.text, fragmentOffsetSdnv.length);
		cursor += fragmentOffsetSdnv.length;
		memcpy(cursor, totalAduLengthSdnv.text, totalAduLengthSdnv.length);
		cursor += totalAduLengthSdnv.length;
	}

	/*	Insert pre-payload extension blocks.			*/

	for (elt = sdr_list_first(bpSdr, bundle->extensions[PRE_PAYLOAD]); elt; elt = sdr_list_next(bpSdr, elt))
	{
	...
	*cursor = 1; /*	payload block type	*/
	...
	{
		blkProcFlagsSdnv.text[blkProcFlagsSdnv.length - 1] |= BLK_IS_LAST;
	}
	...
	oK(zco_prepend_header(bpSdr, bundle->payload.content, (char * ) buffer, totalHeaderLength));
	MRELEASE(buffer);
	...
	if (totalTrailerLength > 0)
	{
		buffer = (unsigned char *) MTAKE(totalTrailerLength);
		if (buffer == NULL)
		{
		...
		cursor = buffer;
		for (elt = sdr_list_first(bpSdr, bundle->extensions[POST_PAYLOAD]); elt; elt = nextElt)
		{
			nextElt = sdr_list_next(bpSdr, elt);
			blkAddr = sdr_list_data(bpSdr, elt);
			sdr_read(bpSdr, (char *) &blk, blkAddr, sizeof(ExtensionBlock));
			...
			for (i = 1, flagbyte = cursor + 1; i <= blk.length; i++, flagbyte++)
			{
				if (((*flagbyte) & 0x80) == 0)
				{
					if (nextElt == 0)
		...
		oK(zco_append_trailer(bpSdr, bundle->payload.content, (char * ) buffer, totalTrailerLength));
		MRELEASE(buffer);
	}
...
static int insertNonCbheCustodian(Bundle *bundle, VScheme *vscheme)
{
	...
	bundle->destination.d.schemeNameOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, string, stringLength);
	string = oldDictionary + bundle->destination.d.nssOffset;
	stringLength = strlen(string);
	bundle->destination.d.nssOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, string, stringLength);
	string = oldDictionary + bundle->id.source.d.schemeNameOffset;
	stringLength = strlen(string);
	bundle->id.source.d.schemeNameOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, string, stringLength);
	string = oldDictionary + bundle->id.source.d.nssOffset;
	stringLength = strlen(string);
	bundle->id.source.d.nssOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, string, stringLength);
	string = oldDictionary + bundle->reportTo.d.schemeNameOffset;
	stringLength = strlen(string);
	bundle->reportTo.d.schemeNameOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, string, stringLength);
	string = oldDictionary + bundle->reportTo.d.nssOffset;
	stringLength = strlen(string);
	bundle->reportTo.d.nssOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, string, stringLength);
	bundle->custodian.d.schemeNameOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, vscheme->adminEid, vscheme->nameLength);
	bundle->custodian.d.nssOffset = addStringToDictionary(strings, stringLengths, &stringCount, &bundle->dictionaryLength, vscheme->adminEid + (vscheme->nameLength + 1), vscheme->adminNSSLength);

	/*	Now concatenate all of these strings into the new
	 *	dictionary.						*/

	newDictionary = (char *) MTAKE(bundle->dictionaryLength);
	...
	MRELEASE(oldDictionary);
	sdr_write(bpSdr, bundle->dictionary, newDictionary, bundle->dictionaryLength);
	MRELEASE(newDictionary);
	bundle->dbOverhead += bundle->dictionaryLength;
	return 0;
}
...
Object bundleAddr, int priority, unsigned char ordinal, time_t enqueueTime)
{
	Sdr bpSdr = getIonsdr();
...
Object bundleObj, int backlogIncrement)
{
	...
	{
		xmitElt = sdr_list_insert_first(getIonsdr(), duct->urgentQueue, bundleObj);
	}
	else /*	Enqueue after this one.			*/
	{
		xmitElt = insertBundleIntoQueue(duct->urgentQueue, lastElt, bundleObj, 2, ordinal, bundle->enqueueTime);
	}
...
int bpEnqueue(FwdDirective *directive, Bundle *bundle, Object bundleObj, char *proxNodeEid)
{
...
int enqueueToLimbo(Bundle *bundle, Object bundleObj)
{
	...
	bundle->ductXmitElt = sdr_list_insert_last(bpSdr, bpConstants->limboQueue, bundleObj);
	sdr_write(bpSdr, bundleObj, (char * ) bundle, sizeof(Bundle));
...
int reverseEnqueue(Object xmitElt, ClProtocol *protocol, Object outductObj, Outduct *outduct, int sendToLimbo)
{
	Sdr bpSdr = getIonsdr();
	Object bundleAddr;
	Bundle bundle;
	...
	removeBundleFromQueue(&bundle, bundleAddr, protocol, outductObj, outduct);
	if (bundle.proxNodeEid)
	{
		sdr_free(bpSdr, bundle.proxNodeEid);
		bundle.proxNodeEid = 0;
...
int bpBlockOutduct(char *protocolName, char *ductName)
{
	...
	for (xmitElt = sdr_list_first(bpSdr, outduct.urgentQueue); xmitElt; xmitElt = nextElt)
	{
	...
	for (xmitElt = sdr_list_first(bpSdr, outduct.stdQueue); xmitElt; xmitElt = nextElt)
	{
		nextElt = sdr_list_next(bpSdr, xmitElt);
		if (reverseEnqueue(xmitElt, &protocol, outductObj, &outduct, 0))
		{
	...
	for (xmitElt = sdr_list_first(bpSdr, outduct.bulkQueue); xmitElt; xmitElt = nextElt)
	{
		nextElt = sdr_list_next(bpSdr, xmitElt);
		if (reverseEnqueue(xmitElt, &protocol, outductObj, &outduct, 0))
		{
...
int bpUnblockOutduct(char *protocolName, char *ductName)
{
	...
	for (xmitElt = sdr_list_first(bpSdr, bpConstants->limboQueue); xmitElt; xmitElt = nextElt)
	{
		nextElt = sdr_list_next(bpSdr, xmitElt);
...
int bpAbandon(Object bundleObj, Bundle *bundle, int reason)
{
	...
	else
	{
		...
		{
			bpCtTally(ctReason, bundle->payload.length);
			result2 = noteCtEvent(bundle, NULL, dictionary, 0, ctReason);
			if (result2 < 0)
			{
				putErrmsg("Can't send custody signal.", NULL);
			}
		...
		else /*	Bundle is not custodial.		*/
		{
			...
			result2 = sendCtSignal(bundle, dictionary, 0, CtNoKnownRoute);
			if (result2 < 0)
			{
				putErrmsg("Can't send custody signal.", NULL);
			}
...
static void selectNextBundleForTransmission(Outflow *flows, Outflow **winner, Object *eltp)
{
	int i;
	Outflow *flow;
	Object elt;
...
static int getOutboundBundle(Outflow *flows, VOutduct *vduct, Outduct *outduct, Object outductObj, ClProtocol *protocol, unsigned int maxPayloadLength,
Object *bundleObj, Bundle *bundle)
{
	Sdr bpSdr = getIonsdr();
	PsmPartition bpwm = getIonwm();
	...
	while (1) /*	Might do one or more reforwards.	*/
	{
		...
		if (xmitElt == 0) /*	Nothing ready.	*/
		{
			...
			{
				putErrmsg("CLO can't take duct semaphore.", NULL);
				return -1;
			}
		...
		*bundleObj = sdr_list_data(bpSdr, xmitElt);
		...
		if (maxPayloadLength > 0 && bundle->payload.length > maxPayloadLength)
		{
			...
			if (bpClone(bundle, &firstBundle, &firstBundleObj, 0, maxPayloadLength) < 0 || bpClone(bundle, &secondBundle, &secondBundleObj, maxPayloadLength, bundle->payload.length - maxPayloadLength) < 0)
			{
				putErrmsg("CLO can't fragment bundle.", NULL);
				return -1;
			}
			...
			if (bpDestroyBundle(*bundleObj, 0) < 0)
			{
				putErrmsg("CLO can't destroy original bundle.", NULL);
				return -1;
			}

			noteFragmentation(&secondBundle);
			secondBundle.ductXmitElt = sdr_list_insert_first(bpSdr, sourceFlow->outboundBundles, secondBundleObj);
			sdr_write(bpSdr, secondBundleObj, (char * ) &secondBundle, sizeof(Bundle));
			firstBundle.ductXmitElt = sdr_list_insert_first(bpSdr, sourceFlow->outboundBundles, firstBundleObj);
			sdr_write(bpSdr, firstBundleObj, (char * ) &firstBundle, sizeof(Bundle));
		...
		if (bundle->destination.cbhe && bundle->destination.unicast && strcmp(vduct->protocolName, "ltp") == 0)
		{
			neighborNodeNbr = strtoul(vduct->ductName, NULL, 0);
			destNode = findNode(getIonVdb(), bundle->destination.c.nodeNbr, &nextNode);
			if (destNode)
			{
				...
				for (embElt = sm_list_first(bpwm, destNode->embargoes); embElt; embElt = nextEmbargo)
				{
					nextEmbargo = sm_list_next(bpwm, embElt);
					embargo = (Embargo *) psp(bpwm, sm_list_data(bpwm, embElt));
				...
				if (embElt)
				{
					...
					if (bpReforwardBundle(*bundleObj) < 0)
					{
						putErrmsg("Embargo refwd n/g.", NULL);
						return -1;
					}
...
int bpDequeue(VOutduct *vduct, Outflow *flows, Object *bundleZco, BpExtendedCOS *extendedCOS, char *destDuctName, unsigned int maxPayloadLength, int timeoutInterval)
{
	Sdr bpSdr = getIonsdr();
	...
	*destDuctName = '\0'; /*	Default behavior.	*/
	...
	if (getOutboundBundle(flows, vduct, &outduct, outductObj, protocol, maxPayloadLength, &bundleObj, &bundle) < 0)
	{
	...
	*bundleZco = bundle.payload.content;
	bundle.payload.content = zco_clone(bpSdr, *bundleZco, 0, zco_source_data_length(bpSdr, *bundleZco));
	...
	{
		sdr_read(bpSdr, (char *) &bset, sdr_hash_entry_value(bpSdr, (_bpConstants())->bundles, bundle.hashEntry), sizeof(BundleSet));
	...
	bpXmitTally(COS_FLAGS(bundle.bundleProcFlags) & 0x03, bundle.payload.length);
	...
	xmitLength = computeECCC(bundle.payload.length + NOMINAL_PRIMARY_BLKSIZE, protocol);
	throttle->capacity -= xmitLength;

	/*	Return the outbound buffer's extended class of service.	*/

	memcpy((char *) extendedCOS, (char *) &bundle.extendedCOS, sizeof(BpExtendedCOS));
...
static int nextBlock(Sdr sdr, ZcoReader *reader, unsigned char *buffer, int *bytesBuffered, int bytesParsed)
{
	int bytesToReceive;
	int bytesReceived;
	...
	*bytesBuffered -= bytesParsed;
	...
	bytesReceived = zco_transmit(sdr, reader, bytesToReceive, ((char *) buffer) + *bytesBuffered);
	if (bytesReceived < 0)
	{
...
static int bufAdvance(int length, unsigned int *bundleLength, unsigned char **cursor, unsigned char *endOfBuffer)
{
	*cursor += length;
...
static int decodeHeader(Sdr sdr, ZcoReader *reader, unsigned char *buffer, int bytesBuffered, Bundle *image, char **dictionary, unsigned int *bundleLength)
{
	...
	{
		*bundleLength = 0;
		writeMemoNote("[?] Truncated bundle", itoa(endOfBuffer - (cursor + image->dictionaryLength)));
		return 0;
	}
	...
	while (1)
	{
		if (nextBlock(sdr, reader, buffer, &bytesBuffered, cursor - buffer) < 0)
		{
			return -1;
		}
		...
		if (bufAdvance(blockDataLength, bundleLength, &cursor, endOfBuffer) == 0)
		{
			return 0;
		}
	}
...
int decodeBundle(Sdr sdr, Object zco, unsigned char *buffer, Bundle *image, char **dictionary, unsigned int *bundleLength)
{
	ZcoReader reader;
	int bytesBuffered;
	...
	*bundleLength = 0; /*	Initialize to default.		*/
	...
	CHKERR(sdr_begin_xn(sdr));
	bytesBuffered = zco_transmit(sdr, &reader, BP_MAX_BLOCK_SIZE, (char *) buffer);
	if (bytesBuffered < 0)
	{
	...
	if (decodeHeader(sdr, &reader, buffer, bytesBuffered, image, dictionary, bundleLength) < 0)
	{
...
int bpIdentify(Object bundleZco, Object *bundleObj)
{
	...
	*bundleObj = 0; /*	Default: not located.	*/
	...
	if (decodeBundle(bpSdr, bundleZco, buffer, &image, &dictionary, &bundleLength) < 0)
	{
		MRELEASE(buffer);
	...
	CHKERR(sdr_begin_xn(bpSdr)); /*	Just to lock memory.	*/
	result = findBundle(sourceEid, &image.id.creationTime, image.id.fragmentOffset, image.totalAduLength == 0 ? 0 : image.payload.length, bundleObj);
	sdr_exit_xn(bpSdr);
	MRELEASE(sourceEid);
	if (result < 0)
	{
...
{
	isignal(SIGTERM, (void (*)(int)) shutDownAdminApp);sm_SemEnd((_bpadminSap(NULL))->recvSemaphore);
}
...
static void noteEmbargo(Bundle *bundle, Object bundleAddr, char *neighborEid)
{
	IonVdb *ionVdb;
	IonNode *node;
	PsmAddress nextNode;
	...
	if (parseEidString(neighborEid, &metaEid, &vscheme, &vschemeElt) == 0 || !metaEid.cbhe) /*	Non-CBHE, somehow.		*/
	{
...
static void forgetEmbargo(Bundle *bundle, Object bundleAddr, char *neighborEid)
{
	IonVdb *ionVdb;
	IonNode *node;
	PsmAddress nextNode;
...
int applyCtSignal(BpCtSignal *cts, char *bundleSourceEid)
{
	...
	CHKERR(sdr_begin_xn(bpSdr));
	if (findBundle(cts->sourceEid, &cts->creationTime, cts->fragmentOffset, cts->fragmentLength, &bundleAddr) < 0)
	{
		sdr_exit_xn(bpSdr);
	...
	else /*	Custody refused; try again.			*/
	{
		...
		if (result < 0)
		{
			putErrmsg("Can't re-queue bundle for forwarding.", NULL);
			sdr_cancel_xn(bpSdr);
			return -1;
		}
...
int _handleAdminBundles(char *adminEid, StatusRptCB handleStatusRpt, CtSignalCB handleCtSignal)
{
	int running = 1;
	BpSAP sap;
	BpDelivery dlv;
	...
	oK(_bpadminSap(&sap));
	isignal(SIGTERM, (void (*)(int)) shutDownAdminApp);while
(	running && !(sm_SemEnded(sap->recvSemaphore)))
	{
