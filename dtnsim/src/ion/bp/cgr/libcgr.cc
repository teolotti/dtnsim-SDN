...
typedef struct
{
	...
	Scalar protected_; /*	Bytes not overbooked.	*/
...
static void clearRoutingObjects(PsmPartition ionwm)
{
	...
	for (elt = sm_rbt_first(ionwm, ionvdb->nodes); elt; elt = sm_rbt_next(ionwm, elt))
	{
...
static CgrVdb *getCgrVdb()
{
	...
	if ((vdb->routeLists = sm_list_create(ionwm)) == 0 || psm_catlg(ionwm, name, vdbAddress) < 0)
	{
		sdr_exit_xn(sdr);
...
static int getApplicableRange(IonCXref *contact, unsigned int *owlt)
{
	...
	for (oK(sm_rbt_search(ionwm, ionvdb->rangeIndex, rfx_order_ranges, &arg, &elt)); elt; elt = sm_rbt_next(ionwm, elt))
	{
...
static int computeDistanceToTerminus(IonCXref *rootContact, CgrContactNote *rootWork, IonNode *terminusNode, CgrRoute *route, CgrTrace *trace)
{
	PsmPartition ionwm = getIonwm();
	IonVdb *ionvdb = getIonVdb();
	IonCXref *current;
	...
	while (1)
	{
		...
		for (oK(sm_rbt_search(ionwm, ionvdb->contactIndex, rfx_order_contacts, &arg, &elt)); elt; elt = sm_rbt_next(ionwm, elt))
		{
			...
			if (work->capacity == 0)
			{
				work->capacity = contact->xmitRate * (contact->toTime - contact->fromTime);
			}
			...
			if (arrivalTime < work->arrivalTime)
			{
				...
				{
					if (contact->confidence > highestConfidence || (contact->confidence == highestConfidence && work->arrivalTime < earliestFinalArrivalTime))
					{
						highestConfidence = contact->confidence;
						earliestFinalArrivalTime = work->arrivalTime;
		...
		for (elt = sm_rbt_first(ionwm, ionvdb->contactIndex); elt; elt = sm_rbt_next(ionwm, elt))
		{
			contact = (IonCXref *) psp(ionwm, sm_rbt_data(ionwm, elt));
			CHKERR(contact);
			work = (CgrContactNote *) psp(ionwm, contact->routingObject);
		...
		current = nextContact;
		currentWork = (CgrContactNote *) psp(ionwm, nextContact->routingObject);
	}
	...
	if (finalContact) /*	Found a route to terminus node.	*/
	{
		...
		maxCapacity = (uvast) -1;
		for (contact = finalContact; contact != rootContact; contact = work->predecessor)
		{
...
static int findNextBestRoute(PsmPartition ionwm, IonCXref *rootContact, CgrContactNote *rootWork, IonNode *terminusNode, PsmAddress *routeAddr, CgrTrace *trace)
{
	PsmAddress addr;
	CgrRoute *route;
	...
	*routeAddr = 0; /*	Default.			*/
	...
	if (computeDistanceToTerminus(rootContact, rootWork, terminusNode, route, trace) < 0)
	{
...
static PsmAddress loadRouteList(IonNode *terminusNode, time_t currentTime, CgrTrace *trace)
{
	PsmPartition ionwm = getIonwm();
	IonVdb *ionvdb = getIonVdb();
	...
	oK(sm_list_user_data_set(ionwm, terminusNode->routingObject, psa(ionwm, terminusNode)));
	...
	{
		contact = (IonCXref *) psp(ionwm, sm_rbt_data(ionwm, elt));
		if ((work = (CgrContactNote *) psp(ionwm, contact->routingObject)) == 0)
		{
	...
	while (1)
	{
		if (findNextBestRoute(ionwm, &rootContact, &rootWork, terminusNode, &routeAddr, trace) < 0)
		{
		...
		firstContact = (IonCXref *) psp(ionwm, sm_list_data(ionwm, sm_list_first(ionwm, route->hops)));
		if (anchorContact)
		{
			...
			if (firstContact != anchorContact)
			{
				...
				work = (CgrContactNote *) psp(ionwm, anchorContact->routingObject);
				work->suppressed = 1;
				anchorContact = NULL;
				continue;
			}
		...
		else
		{
			...
			for (elt = sm_list_first(ionwm, route->hops); elt; elt = sm_list_next(ionwm, elt))
			{
		...
		work = (CgrContactNote *) psp(ionwm, limitContact->routingObject);
		work->suppressed = 1;
...
static int recomputeRouteForContact(uvast contactToNodeNbr, time_t contactFromTime, IonNode *terminusNode, time_t currentTime, CgrTrace *trace)
{
	...
	for (cxelt = sm_rbt_first(ionwm, vdb->contactIndex); cxelt; cxelt = sm_rbt_next(ionwm, cxelt))
	{
		contact = (IonCXref *) psp(ionwm, sm_rbt_data(ionwm, cxelt));
		if ((work = (CgrContactNote *) psp(ionwm, contact->routingObject)) == 0)
		{
			...
			if (work == 0)
			{
				putErrmsg("Can't create CGR contact note.", NULL);
				return -1;
			}
		}
	...
	for (elt = sm_list_first(ionwm, routes); elt; elt = sm_list_next(ionwm, elt))
	{
		route = (CgrRoute *) psp(ionwm, sm_list_data(ionwm, elt));
		if (route->toNodeNbr == contactToNodeNbr && route->fromTime == contactFromTime)
		{
...
static time_t computeArrivalTime(CgrRoute *route, Bundle *bundle, time_t currentTime, Outduct *outduct, Scalar *overbooked, Scalar *protected_, time_t *eto)
{
	Sdr sdr = getIonsdr();
	PsmPartition ionwm = getIonwm();
	IonVdb *vdb = getIonVdb();
	...
	computePriorClaims(&protocol, outduct, bundle, &priorClaims, &totalBacklog);
	copyScalar(protected_, &totalBacklog);
	...
	arg.toNode = route->toNodeNbr;
	for (oK(sm_rbt_search(ionwm, vdb->contactIndex, rfx_order_contacts, &arg, &elt)); elt; elt = sm_rbt_next(ionwm, elt))
	{
		contact = (IonCXref *) psp(ionwm, sm_rbt_data(ionwm, elt));
		if (contact->fromNode > ownNodeNbr || contact->toNode > route->toNodeNbr || contact->fromTime > route->fromTime)
		{
		...
		subtractFromScalar(&allotment, protected_);
	...
	transmitTime += ((ONE_GIG * radiationLatency.gigs) + radiationLatency.units);
	*eto = transmitTime;
...
Object plans, CgrLookupFn getDirective, CgrTrace *trace, Lyst proximateNodes)
{
	Sdr sdr = getIonsdr();
	PsmPartition ionwm = getIonwm();
	FwdDirective directive;
	...
	sdr_read(sdr, (char *) &outduct, sdr_list_data(sdr, directive.outductElt), sizeof(Outduct));
	if (outduct.blocked)
	{
	...
	if (bundle->bundleProcFlags & BDL_DOES_NOT_FRAGMENT && outduct.maxPayloadLen != 0)
	{
	...
	arrivalTime = computeArrivalTime(route, bundle, currentTime, &outduct, &overbooked, &protected_, &eto);
	...
	for (elt2 = lyst_first(proximateNodes); elt2; elt2 = lyst_next(elt2))
	{
		...
		if (proxNode->neighborNodeNbr == route->toNodeNbr)
		{
			...
			if (route->arrivalConfidence > proxNode->arrivalConfidence || (route->arrivalConfidence == proxNode->arrivalConfidence && arrivalTime < proxNode->arrivalTime))
			{
				...
				proxNode->hops = route->hops;
				copyScalar(&proxNode->overbooked, &overbooked);
				copyScalar(&proxNode->protected_, &protected_);
				TRACE(CgrUpdateProximateNode, CgrLaterArrivalTime);
			}
			else
			{
				if (route->arrivalConfidence == proxNode->arrivalConfidence && arrivalTime == proxNode->arrivalTime)
				{
	...
	if (proxNode == NULL || lyst_insert_last(proximateNodes, (void *) proxNode) == 0)
	{
	...
	memcpy((char *) &(proxNode->directive), (char *) &directive, sizeof(FwdDirective));
	proxNode->arrivalConfidence = route->arrivalConfidence;
	proxNode->arrivalTime = arrivalTime;
	proxNode->hopCount = hopCount;
	proxNode->forfeitTime = route->toTime;
...
Object bundleObj, Lyst excludedNodes, Object plans, CgrLookupFn getDirective, CgrTrace *trace, Lyst proximateNodes, time_t currentTime)
{
	PsmPartition ionwm = getIonwm();
	...
	{
		if ((routes = loadRouteList(terminusNode, currentTime, trace)) == 0)
		{
			putErrmsg("Can't load routes for node.", utoa(terminusNode->nodeNbr));
			return -1;
		}
	}
	...
	for (elt = sm_list_first(ionwm, routes); elt; elt = nextElt)
	{
		...
		if (route->toTime < currentTime)
		{
			...
			psm_free(ionwm, addr);
			sm_list_delete(ionwm, elt, NULL, NULL);
			switch (recomputeRouteForContact(contactToNodeNbr, contactFromTime, terminusNode, currentTime, trace))
			{
			case -1:
		...
		{
			if (!(bundle->destination.cbhe && bundle->destination.c.nodeNbr == route->toNodeNbr))
			{
		...
		if (tryRoute(route, currentTime, bundle, plans, getDirective, trace, proximateNodes) < 0)
		{
...
static float getNewDlvConfidence(Bundle *bundle, ProximateNode *proxNode)
{
	...
	dlvFailureConfidence = (1.0 - bundle->dlvConfidence) * (1.0 - proxNode->arrivalConfidence);
...
Object bundleObj, IonNode *terminusNode)
{
	...
	isprintf(terminusEid, sizeof terminusEid, "ipn:" UVAST_FIELDSPEC ".%u", proxNode->neighborNodeNbr, serviceNbr);
	...
	ionwm = getIonwm();
	for (embElt = sm_list_first(ionwm, terminusNode->embargoes); embElt; embElt = sm_list_next(ionwm, embElt))
	{
	...
	// We don't enable this behaviour in the simulator
...
static int manageOverbooking(ProximateNode *neighbor, Object plans, Bundle *newBundle, CgrLookupFn getDirective, CgrTrace *trace)
{
	Sdr sdr = getIonsdr();
	QueueControl queueControls[] =
	{
	{ 0, 0 },
	{ 0, 0 },
	{ 0, 0 } };
	...
	overbooked += (ONE_GIG * neighbor->overbooked.gigs) + neighbor->overbooked.units;
	if (overbooked == 0.0)
	{
		return 0; /*	No overbooking to manage.	*/
	}

	protected_ += (ONE_GIG * neighbor->protected_.gigs) + neighbor->protected_.units;
	if (protected_ == 0.0)
	{
	...
	if (getDirective(neighbor->neighborNodeNbr, plans, newBundle, &directive) == 0)
	{
		TRACE(CgrIgnoreRoute, CgrNoApplicableDirective);
	...
	if (priority > 1)
	{
		queueControls[1].currentElt = sdr_list_last(sdr, outduct.stdQueue);
		queueControls[1].limitElt = sdr_list_first(sdr, outduct.stdQueue);
		ordinal = bundle->extendedCOS.ordinal;
		if (ordinal > 0)
		{
			queueControls[2].currentElt = sdr_list_last(sdr, outduct.urgentQueue);
			queueControls[2].limitElt = getUrgentLimitElt(&outduct, ordinal);
		}
	}
...
static int sendCriticalBundle(Bundle *bundle, Object bundleObj, IonNode *terminusNode, Lyst proximateNodes, int preview)
{
	LystElt elt;
	LystElt nextElt;
	ProximateNode *proxNode;
	...
	for (elt = lyst_first(proximateNodes); elt; elt = nextElt)
	{
		...
		if (enqueueToNeighbor(proxNode, bundle, bundleObj, terminusNode))
		{
	...
	lyst_destroy(proximateNodes);
	if (bundle->dlvConfidence >= MIN_NET_DELIVERY_CONFIDENCE || bundle->id.source.c.nodeNbr == bundle->destination.c.nodeNbr)
	{
...
static int cgrForward(Bundle *bundle, Object bundleObj, uvast terminusNodeNbr, Object plans, CgrLookupFn getDirective, time_t atTime, CgrTrace *trace, int preview)
{
	IonVdb *ionvdb = getIonVdb();
	CgrVdb *cgrvdb = getCgrVdb();
	...
	if (identifyProximateNodes(terminusNode, bundle, bundleObj, excludedNodes, plans, getDirective, trace, proximateNodes, atTime) < 0)
	{
	...
	if (bundle->extendedCOS.flags & BP_MINIMUM_LATENCY)
	{
		...
		return sendCriticalBundle(bundle, bundleObj, terminusNode, proximateNodes, preview);
	}
	...
	for (elt = lyst_first(proximateNodes); elt; elt = nextElt)
	{
		...
		{
			newDlvConfidence = getNewDlvConfidence(bundle, proxNode);
			confidenceImprovement = (newDlvConfidence / bundle->dlvConfidence) - 1.0;
		...
		else if (proxNode->arrivalConfidence > selectedNeighbor->arrivalConfidence)
		{
			TRACE(CgrSelectProximateNode);
		...
		else if (proxNode->arrivalTime < selectedNeighbor->arrivalTime)
		{
			TRACE(CgrSelectProximateNode);
			MRELEASE(selectedNeighbor);
			selectedNeighbor = proxNode;
		...
		else if (proxNode->neighborNodeNbr < selectedNeighbor->neighborNodeNbr)
		{
			TRACE(CgrSelectProximateNode);
			MRELEASE(selectedNeighbor);
			selectedNeighbor = proxNode;
	...
	if (selectedNeighbor)
	{
		TRACE(CgrUseProximateNode, selectedNeighbor->neighborNodeNbr, selectedNeighbor->hops);
		if (!preview)
		{
			if (enqueueToNeighbor(selectedNeighbor, bundle, bundleObj, terminusNode))
			{
				putErrmsg("Can't queue for neighbor.", NULL);
				return -1;
			}
			...
			if (manageOverbooking(selectedNeighbor, plans, bundle, getDirective, trace))
			{
...
int cgr_preview_forward(Bundle *bundle, Object bundleObj, uvast terminusNodeNbr, Object plans, CgrLookupFn getDirective, time_t atTime, CgrTrace *trace)
{
	if (cgrForward(bundle, bundleObj, terminusNodeNbr, plans, getDirective, atTime, trace, 1) < 0)
	{
...
{
	if (cgrForward(bundle, bundleObj, terminusNodeNbr, plans, getDirective, getUTCTime(), trace, 0) < 0)
	{
		putErrmsg("Can't compute route.", NULL);
		return -1;
	}
