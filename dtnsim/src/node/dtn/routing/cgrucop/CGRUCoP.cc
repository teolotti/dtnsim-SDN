...
#include <cgrucop/CGRUCoP.h>

CGRUCoP::CGRUCoP(int eid, SdrModel * sdr, ContactPlan * contactPlan, bool printDebug, int max_copies, double probability_of_failure, int ts_duration, std::vector<int> ts_start_times, int numOfNodes, string pathPrefix, string pathPosfix) :
		RoutingDeterministic(eid, sdr, contactPlan)
{
	...
	for(int source=1; source <= numOfNodes; source++)
	{
		for(int target=1; target <= numOfNodes; target++)
		{
			if (source != target)
			{
				for(int copies=1; copies<=max_copies; copies++)
				{
					ostringstream stream;
					stream << pathPrefix << source-1 << "-" << target-1 << "-" << copies << pathPosfix;
					...
					if (infile.good())
					{
						...
						this->bruf_function[to_string(source)][to_string(target)][to_string(copies)] = json::parse(infile);
...
void CGRUCoP::routeAndQueueBundle(BundlePkt * bundle, double simTime)
{
...
void CGRUCoP::cgrForward(BundlePkt * bundle, double simTime)
{
...
void CGRUCoP::identifyProximateNodes(BundlePkt * bundle, double simTime, vector<int> excludedNodes, vector<ProximateNode> * proximateNodes)
{
...
void CGRUCoP::tryRoute(BundlePkt * bundle, CgrRoute * route, vector<ProximateNode> * proximateNodes)
{
...
void CGRUCoP::loadRouteList(int terminusNode, double simTime)
{
...
void CGRUCoP::findNextBestRoute(Contact * rootContact, int terminusNode, CgrRoute * route)
{
	// increment counter
	dijkstraCalls++;
...
void CGRUCoP::recomputeRouteForContact()
{
	//cout << "***RecomputeRouteForContact not implemented yet!, ignoring route***" << endl;
}

void CGRUCoP::enqueueToNeighbor(BundlePkt * bundle, ProximateNode * selectedNeighbor)
{
...
void CGRUCoP::enqueueToLimbo(BundlePkt * bundle)
{
...
void CGRUCoP::bpEnqueue(BundlePkt * bundle, ProximateNode * selectedNeighbor)
{
...
CgrRoute* CGRUCoP::getCgrBestRoute(BundlePkt * bundle, double simTime)
{
...
vector<CgrRoute> CGRUCoP::getCgrRoutes(BundlePkt * bundle, double simTime)
{
	if (!printDebug_) // disable cout if degug disabled
		cout.setstate(std::ios_base::failbit);
...
int CGRUCoP::getDijkstraCalls()
{
	return dijkstraCalls;
}

int CGRUCoP::getDijkstraLoops()
{
	return dijkstraLoops;
}

int CGRUCoP::getRouteTableEntriesExplored()
{
	return tableEntriesExplored;
}

double CGRUCoP::get_node_future_delivery_probability(int source_eid, int target_eid, int carrier_eid, int ts)
{
...
double CGRUCoP::get_probability_if_this_route_is_chosed(int source_eid, int target_eid, CgrRoute * route)
{
...
int CGRUCoP::get_ts_for_contact(Contact * c){
