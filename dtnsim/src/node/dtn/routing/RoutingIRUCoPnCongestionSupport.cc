/*
 * RoutingIRUCoPnCongestionSupport.cpp
 *
 *  Created on: February 19, 2020
 *      Author: FRaverta
 */

#include <dtn/routing/RoutingIRUCoPnCongestionSupport.h>

RoutingIRUCoPnCongestionSupport::RoutingIRUCoPnCongestionSupport(int eid, SdrModel * sdr, ContactPlan * contactPlan,  cModule * dtn, int max_copies, double probability_of_failure, int ts_duration, int numOfNodes, string pathPrefix, string pathPosfix)
	...
	for(int target=1; target <= numOfNodes; target++)
	{
		if (eid_ != target)
		{
			...
			for(copies=1; copies<=max_copies; copies++)
			{
				...
				cout<<"RoutingIRUCoPnCongestionSupport::RoutingIRUCoPnCongestionSupport check if exist file: " << pathToRouting << endl;
				...
				if (infile.good())
				{
					...
					cout<<"RoutingIRUCoPnCongestionSupport::RoutingIRUCoPnCongestionSupport there is not traffic destined to " << target << endl;
					break;
					//exit(1);
				}
			}
			...
			if (this->bruf_function.find(to_string(target)) != this->bruf_function.end())
			{
				for(copies=1; copies<=max_copies; copies++)
				{
					...
					{
						cout<<"[Error] in RoutingIRUCoPnCongestionSupport::RoutingIRUCoPnCongestionSupport() : function destined to "<< target<<" with "<<copies << " copies should exist"<<endl;
						exit(1);
					}
				}
			}
...
RoutingIRUCoPnCongestionSupport::~RoutingIRUCoPnCongestionSupport()
{
	cout<<"Node "<<eid_<<" Calling RoutingIRUCoPnCongestionSupport::~RoutingIRUCoPnCongestionSupport()"<<endl;
	if (bag.size() != 0)
		cout<<"RoutingIRUCoPnCongestionSupport::~RoutingIRUCoPnCongestionSupport: Node "<<eid_<<" has "<<bag.size()<<" bundles in its bag but It should not have anyone"<<endl;

}

void RoutingIRUCoPnCongestionSupport::msgToOtherArrive(BundlePkt * bundle, double simTime)
{
	...
	{
		cout<<"[Error] in RoutingIRUCoPnCongestionSupport::msgToOtherArrive there is not function loaded for routing traffic destined to  " << bundle->getDestinationEid() <<endl;
		exit(1);
	}
	...
	{
		cout<<"RoutingIRUCoPnCongestionSupport::msgToOtherArrive Node " << eid_ << ": Bundle " << bundle << "( "<< max_copies << " copies) was generated."<< endl;
		bundle->setBundlesCopies(max_copies); // Set the number of copies to current bundle
	}
	routeAndQueueBundle(bundle, simTime);
}

bool RoutingIRUCoPnCongestionSupport::msgToMeArrive(BundlePkt * bundle)
{
...
void RoutingIRUCoPnCongestionSupport::contactStart(Contact *c)
{
	cout<<"RoutingIRUCoPnCongestionSupport::contactStart "<<c->getId()<<endl;
...
void RoutingIRUCoPnCongestionSupport::contactEnd(Contact *c)
{
	cout<<"RoutingIRUCoPnCongestionSupport::contactEnd: "<<c->getId()<<endl;
...
void RoutingIRUCoPnCongestionSupport::routeAndQueueBundle(BundlePkt * bundle, double simTime)
{
	...
	if (it != this->bag.end())
	{
		...
		{
			cout<<"Error RoutingIRUCoPnCongestionSupport::routeAndQueueBundle: Node "<< eid_ <<" tries to enqueue a bundle that have been already enqueued"<<endl;
			exit(1);
		}
	...
	{
		cout<<"RoutingIRUCoPnCongestionSupport::routeAndQueueBundl searching "<<source <<" - "<<target<<" - "<<copies<<" - "<<ts<<" - "<<key<<endl;
	...
	if (routing_decisions.size() == 0)
	{
		...
		cout<<"RoutingIRUCoPnCongestionSupport::routeAndQueueBundle bundle source: "<<source<<" - target: "<< target <<" - copies: "<< copies << " has been dropped at Node "<< eid_ <<" - Ts: "<< int( simTime / this->ts_duration)<<" = "<<simTime<<"/"<<ts_duration <<endl;
	}else{
		it->second = routing_decisions;
		cout<<"RoutingIRUCoPnCongestionSupport::routeAndQueueBundle bundle source: "<<source<<" target: "<< target << " copies: "<< copies << " has been routed using contacts: ";
		...
		if (invalidRouting)
		{
			cout<<"RoutingIRUCoPnCongestionSupport::routeAndQueueBundle: Node "<< eid_ <<" routed a bundle through a contact for which it isn't is source "<<endl;
			exit(1);
		}
		if (finishedContact)
		{
			cout<<"RoutingIRUCoPnCongestionSupport::routeAndQueueBundle: Node "<< eid_ <<" routed a bundle through a contact that had finished."<<endl;
			exit(1);
		}
	}
	queueBundles();
...
void RoutingIRUCoPnCongestionSupport::queueBundles()
{
	for(list<int>::iterator contactIt = currentContacts.begin(); contactIt != currentContacts.end(); contactIt++){
		...
		for(map<long, list<int>>::iterator it = bag.begin(); it != bag.end();)
		{
			...
			if (cant > 0)
			{
				...
				RoutingIRUCoPnCongestionSupport * other = check_and_cast<RoutingIRUCoPnCongestionSupport *>(check_and_cast<Dtn *>(
				...
				if (other->isDeliveredBundle(it->first)){
					...
					{
						cout<<"Error in RoutingIRUCoPnCongestionSupport::queueBundles: Node "<< eid_ <<" tries to route more copies than the ones it was carrying."<<endl;
						exit(1);
					}
...
void RoutingIRUCoPnCongestionSupport::removeBundleFromBag(long bundleId){
 ...
 */
void RoutingIRUCoPnCongestionSupport::successfulBundleForwarded(long bundleId, Contact * contact,  bool sentToDestination)
{
 ...
 */
bool RoutingIRUCoPnCongestionSupport::isDeliveredBundle(long bundleId)
{
