 ...
 *****************************************************************************/
PhaseTwoSAP *get_phase_two_sap(PhaseTwoSAP * newSap) { //was static
 ...
 *  06/02/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
static int computeEffectiveVolumeLimit(UniboContact *contact, Priority priority,
		...
		CgrScalar *residualBacklog, time_t *lastByteArrivalTime)
{
	...
	UniboContact *contact, *nextContact;
	ListElt *elt, *nextElt;
	Priority priority;
#if (QUEUE_DELAY == 1)
	...
	contact = (UniboContact*) route->hops->first->data;
	*lastByteArrivalTime = 0;
	...
	if (contact->xmitRate > 0)
	{
		...
		while (contact != NULL && viableRoute)
		{
			...
			nextContact = (nextElt == NULL) ? NULL : (UniboContact*) nextElt->data;
...
#if (NEGLECT_CONFIDENCE == 0)
		firstContact = (UniboContact*) route->hops->first->data;
#endif
