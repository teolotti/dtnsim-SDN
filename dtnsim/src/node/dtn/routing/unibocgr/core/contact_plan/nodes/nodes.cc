 ...
 *****************************************************************************/
Rbt *get_node_graph(Rbt *newNodes) { //was static
	static Rbt* nodes;
 ...
 *  02/07/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
NeighborsSAP *get_neighbors_sap(NeighborsSAP *newSap) { //was static
 ...
 *  15/01/20 | L. Persampieri  |  Initial Implementation and documentation.
 *****************************************************************************/
int create_NodesTree(bool newNode)
{
	int result = 1;
...
{
	int result = 0, stop = 0;
	UniboContact *contact, *prevContact = NULL;
	RbtNode *node;
	NeighborsSAP *sap = get_neighbors_sap(NULL);
	...
	if(!sap->neighbors_list_builded)
	{
		...
		if(sap->local_node_neighbors != NULL)
		{
		    ...
		    {
		        unsigned long * currentRegion = (unsigned long*) elt->data;
