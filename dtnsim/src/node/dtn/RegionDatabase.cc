...
void RegionDatabase::readLine(const string & s) {
	...
	if (a.compare("a") == 0) {
		...
		if ((command.compare("node") == 0)) {
			...
			addNode(nodeEid+1, regionId, rx, tx);
			return;
...
void RegionDatabase::populateVents() {
	...
	for (auto it1 = nodesRxRegions_.begin(); it1 != nodesRxRegions_.end(); ++it1) {
		...
		if (it2 != nodesTxRegions_.end()) {
			...
			// iterate through vents and populate them in the region database (nodesVents_ field)
...
// given a node and its associated set of rx regions and tx regions -> find vents
set<pair<string, string> > RegionDatabase::getCombinations(set<string> s1, set<string> s2) {

	set < pair<string, string> > combinations;

	// iterate through all rx regions
	for (auto it1 = s1.begin(); it1 != s1.end(); ++it1) {

		string rxRegion = *it1;

		// for each rx region, look at all tx regions, insert into combinations if they're *not* the same (cannot vent into same region)
