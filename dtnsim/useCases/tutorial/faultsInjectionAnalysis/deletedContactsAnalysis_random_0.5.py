            ...
            for aware in ["true", "false"]:
                ...
                for cp in CP_RANGE:
                    if aware == "false":
                        ...
                        f_avg_by_rep.append(receivedPacketAv("%s/example5a-CP=contactPlan#2f%1.1f#_%d,faultsAware=%s"%(INPUT_PATH, density, cp,aware),
                                                         MAX_DELETED_CONTACTS,
                                                         AMOUNT_OF_REPETITIONS,
                                                         metric[0]))
                    else:
                        # compute average function for all repetitions of a contact plan (one contac plan average- CONTACT PLAN AVERAGE)
                        f_avg_by_rep.append(receivedPacketAv(
                            "%s/example5b-CP=contactPlan#2f%1.1f#_%d,faultsAware=%s" % (INPUT_PATH, density, cp, aware),
                            MAX_DELETED_CONTACTS,
                            AMOUNT_OF_REPETITIONS,
                            metric[0]))
