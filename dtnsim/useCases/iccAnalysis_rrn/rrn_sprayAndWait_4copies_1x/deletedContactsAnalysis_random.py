...
OUTPUT_PATH = os.getcwd() + "/results/results_random/"

MAX_DELETED_CONTACTS = 2788
            ...
            for aware in ["false"]:
                ...
                for cp in CP_RANGE:
                    ...
                    if(metric[0] == "deliveryRatio"):
                        f_avg_by_rep.append(receivedPacketAv2("%s/dtnsim-faultsAware=%s"%(INPUT_PATH,aware),MAX_DELETED_CONTACTS,AMOUNT_OF_REPETITIONS))
                    elif( (metric[0] == "appBundleReceivedDelay:mean") or (metric[0] == "appBundleReceivedHops:mean") or (metric[0] == "sdrBundleStored:timeavg")):
                        f_avg_by_rep.append(receivedPacketAv3("%s/dtnsim-faultsAware=%s" % (INPUT_PATH,aware),MAX_DELETED_CONTACTS, AMOUNT_OF_REPETITIONS,metric[0]))
                    else:
                        #compute average function for all repetitions of a contact plan (one contac plan average- CONTACT PLAN AVERAGE)
                        f_avg_by_rep.append(receivedPacketAv("%s/dtnsim-faultsAware=%s"%(INPUT_PATH,aware),MAX_DELETED_CONTACTS,AMOUNT_OF_REPETITIONS,metric[0]))
                ...
                # save function
                text_file = open("%s/METRIC=%s-FAULTAWARE=%s-MAX_DELETED_CONTACTS=%d.txt"%(OUTPUT_PATH,metric[0],aware,MAX_DELETED_CONTACTS),"w")
